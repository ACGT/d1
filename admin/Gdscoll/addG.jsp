...
<%!
//获取最大情侣编号
private  static String getGdscollBybgid()
{
	...
	{
		return list.get(0).getGdscoll_bgid().toString();
	}
	else
	{
		return "0";
	  ...
	  String smallimg="";
	    ...
	    Long size=0l;
		  ...
		  if(gs1!=null&&gs1.length()>0&&!Tools.isMath(gs1))
		  {
		    ...
		    gdscoll.setGdscoll_bgid(new Long(Tools.parseLong(ql)));
		    gdscoll=(Gdscoll)Tools.getManager(Gdscoll.class).create(gdscoll);
		    if(gdscoll!=null){
		    	out.print("<script>alert('添加搭配成功！');this.location.href='/admin/Gdscoll/addG.jsp?sid="+gs+"';</script>");
		    }else{
		    	out.print("<script>alert('添加搭配失败！');this.location.href='/admin/Gdscoll/addG.jsp?sid="+gs+"';</script>");
		    }
		   } catch (Exception e) {
			   out.print("fail");
		        e.printStackTrace();
...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px;">
   ...
   <a href="http://www.d1.com.cn/admin/Gdscoll/gmanager.jsp" target="top">搭配管理</a><br/>
   <form id="gdscoll" name="gdscoll" method="post" action="addG.jsp" enctype="multipart/form-data">
   <table style=" margin:0px auto; font-size:14px;text-align:left;">
      
        	<input type="hidden" id="gdscoll_serid" name="gdscoll_serid" value="<%= sid%>" style="width:60px;"/>
        <tr><td>系列编号：</td><td><%= sid %></td></tr>
        <tr><td>搭配主题：</td><td><input type="text" id="gdscoll_title" name="gdscoll_title" />&nbsp;&nbsp;<span id="gdscoll_title_notice"></span></td></tr>
        <tr><td>搭配大图：</td><td><input type="file" id="gdscoll_bigimgurl" name="gdscoll_bigimgurl" /><span>搭配页面上方显示的大图（大小：344*496，背景透明）</span>
        </td></tr>
       
        <tr><td>搭配类别：<select id="gdscoll_cate" name="gdscoll_cate">
                                          <option value="1">男</option>
                                          <option value="2">女</option>
                                          <option value="3">情侣男</option>
                                          <option value="4">情侣女</option>
        ...
        <tr><td>情侣编号：</td><td><input type="text" id="gdscoll_bgid" name="gdscoll_bgid"/><span>当前情侣编号是<font style="color:#000">”<%= getGdscollBybgid() %>“</font></span></td></tr>
