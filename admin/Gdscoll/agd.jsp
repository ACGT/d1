	  ...
	  String endimg="";
	    Long size=0l;
		  ...
		  if(gs1!=null&&gs1.length()>0&&!Tools.isMath(gs1))
		  {
		    ...
		    if(gdscolld!=null){
		    	out.print("<script>alert('添加搭配商品成功！');this.location.href='/admin/Gdscoll/agd.jsp?gdscollid="+gdscollid+"';</script>");
		    }else{
		    	out.print("<script>alert('添加搭配商品失败！');this.location.href='/admin/Gdscoll/agd.jsp?gdscollid="+gdscollid+"';</script>");
		    }
		   } catch (Exception e) {
			   out.print("fail");
		        e.printStackTrace();
...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px;">
   ...
   <a href="/admin/Gdscoll/gdmanager.jsp?gid=<%= gdscollid%>" target="bottom">搭配商品管理</a><br/>
   <form id="gdscolld" name="gdscolld" method="post" action="agd.jsp?gdscollid=<%=gdscollid %>" enctype="multipart/form-data">
