	    ...
	    Long size=0l;
		  ...
		  if(gs1!=null&&gs1.length()>0&&!Tools.isMath(gs1))
		  {
			...
			if(Tools.getManager(Gdscolldetail.class).update(gdscolld, true)){
		    	out.print("<script>alert('修改搭配商品成功！');this.location.href='/admin/Gdscoll/gdu.jsp?id="+id+"';</script>");
		    }else{
		    	out.print("<script>alert('修改搭配商品失败！');this.location.href='/admin/Gdscoll/gdu.jsp?id="+id+"';</script>");
		    }
		   } catch (Exception e) {
			   out.print("fail");
		        e.printStackTrace();
...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px; font-size:14px;" >
   ...
   <%
       ...
       {%><br/><br/>
    	   <div id="updatefl">
    	      <span>修改搭配商品信息</span>
    	      <form id="gdscolldupdate" name="gdscolldupdate" method="post" action="gdu.jsp?id=<%= id %>"  enctype="multipart/form-data">
