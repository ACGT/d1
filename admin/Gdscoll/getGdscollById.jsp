...
<%!
   ...
   private static ArrayList<Gdscoll> GetGdscollById(String id)
   {
    ArrayList<Gdscoll> list=new ArrayList<Gdscoll>();
		...
		List<BaseEntity> blist=Tools.getManager(Gdscoll.class).getList(clist, null, 0, 10000);
...
<%
   ...
   {
	   id=request.getParameter("gdscollid");
	   //System.out.print(request.getParameter("gdscollid"));
   }
...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px; font-size:14px;">
   ...
   <%
      ...
      ArrayList<Gdscoll> list=new ArrayList<Gdscoll>();
      list=null;
      list=GetGdscollById(id);    
      //list=getGdscollByGdsid(id);
    	 ...
    	 if(request.getParameter("pageno1")!=null&&request.getParameter("pageno1").length()>0)
	      ...
	      for(int i=(pageno1-1)*15;i<list.size()&&i<pageno1*15;i++)
    	  {
    		  ...
    		  {%>
    			...
    			<% Gdsser gds=(Gdsser)Tools.getManager(Gdsser.class).get(fl.getGdscoll_serid().toString()); if(gds!=null&&gds.getGdsser_title()!=null) out.print("<td>"+gds.getGdsser_title()+"</td>"); else{out.print("<td></td>");} %>
    			<%}
    			else{out.print("<td></td>");
    			}
    			%>
    			<td><img src="http://images1.d1.com.cn<%= fl.getGdscoll_smallimgurl()!=null?fl.getGdscoll_smallimgurl():"" %>"/></td>
    			<td><%= fl.getGdscoll_title()!=null?fl.getGdscoll_title():"" %></td><td><%= fl.getGdscoll_sort()!=null?fl.getGdscoll_sort():""%></td>
    				<td>
    			<%
    			    if(fl.getGdscoll_flag()!=null&&fl.getGdscoll_flag().longValue()==1)
    			    {
    			    	out.print("显示");
    			    }
    			    else
    			...
    			<td><a href="gcupdate.jsp?id=<%=fl.getId() %>&sid=<%= fl.getGdscoll_serid().toString() %>" target="bottom">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="deleteGdscoll('<%= fl.getId() %>',<%= id%>)">删除</a>
