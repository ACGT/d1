	    ...
	    Long size=0l;
		  ...
		  if(gs1!=null&&gs1.length()>0&&!Tools.isMath(gs1))
		  {
		    ...
		    if(Tools.getManager(Gdsscene.class).update(gdsscene, false)){
		    	Tools.outJs(out, "修改场景成功", "/admin/Gdsscene/gdsscenemanager.jsp");
		    }else{
		    	
		    	Tools.outJs(out, "修改场景失败", "back");
		    }
		   } catch (Exception e) {
			   out.print("fail");
		        e.printStackTrace();
...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px; font-size:14px;" >
   ...
   <a href="addgdsscene.jsp">添加场景</a>&nbsp;&nbsp;<a href="gdsscenemanager.jsp">场景管理</a><br/>
   <%
       ...
       {%><br/><br/>
    	   <div id="updatefl">
    	      <span>修改场景信息</span>
    	      <form id="gdssceneupdate" name="gdssceneupdate" method="post" action="gdssceneupdate.jsp?id=<%= id %>"  enctype="multipart/form-data">
			  ...
			  <tr><td>系列编号：</td><td><input type="text" id="gdsscene_serid" name="gdsscene_serid" style="width:60px;" value="<%= gdsscene.getGdsscene_gdserid() %>"/>&nbsp;&nbsp;<span id="gdsscene_serid_notice"></span></td></tr>
		        ...
		        <tr><td>场景主图：</td><td><input type="file" id="gdsscene_imgurl" name="gdsscene_imgurl" /><a href="http://images1.d1.com.cn<%= gdsscene.getGdsscene_imgurl() %>">查看主图</a>
		        </td></tr>
                ...
                <tr><td>缩略图：</td><td><input type="file" id="gdsscene_scalimg" name="gdsscene_scalimg" /><a href="http://images1.d1.com.cn<%= gdsscene.getGdsscene_scalimg() %>">查看缩略图</a><span>场景的缩略图（请上传80*80的图）</span></td></tr>
