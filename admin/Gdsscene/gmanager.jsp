...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px; font-size:14px;">
   ...
   <a href="add.jsp" target="bottom">添加场景</a><br/>
   <%
      ...
      {%>
      <table style="margin:0px auto;text-align:center; "  border="1" >
         
         <tr style="background:#f4f4f4; color:#333"><td width="50">编号</td><td width="80">系列名称</td><td width="150">场景标题</td><td width="60">排序</td><td width="60">主图</td><td width="60">是否显示</td>
        <td width="120">场景状态</td><td width="80">缩略图标题</td><td width="200">操作</td></tr>
    	 <% 
    	 if(request.getParameter("pageno1")!=null&&request.getParameter("pageno1").length()>0)
	      ...
	      for(int i=(pageno1-1)*15;i<list.size()&&i<pageno1*15;i++)
    	  {
    		  ...
    		  {%>
    			...
    			<td><a href="http://shopimg.d1.com.cn<%= fl.getGdsscene_imgurl()%>" target="_blank">查看图片</a></td>
    			<td>
    			<%
    			...
    			<td><a href="gu.jsp?id=<%=fl.getId() %>" target="bottom">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="deleteGdsscene('<%= fl.getId() %>')">删除</a>&nbsp;&nbsp;
    			<a href="/gdscene/index.jsp?id=<%= fl.getId() %>" target="_blank">预览场景</a>&nbsp;&nbsp;<a href="/gdscene/indexother.jsp?id=<%= fl.getId() %>" target="_blank">预览场景2</a>
    			</td>
    			</tr>  
    		  <%}
    	  }%>
