	  ...
	  String endimg3="";
	    Long size=0l;
		  ...
		  if(gg!=null)
		  {
			 ...
			 // if(endimg3!=null&&endimg3.length()>0)
			 // {
				  //gg.setGdsser_pbg(endimg3);
			 // }
			  String locations="";
			  if(brandid.length()>0)
			  {
				  locations="/admin/Gdsser/gm.jsp"+"?&bid="+brandid;
			  }
			  else
			  {
				  locations="/admin/Gdsser/gm.jsp";
			  }
			  if(Tools.getManager(Gdsser.class).update(gg,true))
			  {
				  out.print("<script>alert('修改成功！');this.location.href='"+locations+"';</script>");
				  
			  }
			  else
			  {
				  out.print("<script>alert('修改失败！');this.location.href='"+locations+"';</script>");
			  }
		  }
		  else
		  {
			  Tools.outJs(out, "该系列不存在，不能修改！","/admin/Gdsser/gm.jsp");
		  }
	  	 } catch (Exception e) {
			   out.print("fail");
		        e.printStackTrace();
...
<div style="margin:0px auto; width:980px; text-align:center; padding-top:25px; font-size:14px;">
   ...
   <%
    	 ...
    	 if(request.getParameter("pageno1")!=null&&request.getParameter("pageno1").length()>0)
	      ...
	      for(int i=(pageno1-1)*15;i<list.size()&&i<pageno1*15;i++)
    	  {
    		  ...
    		  {%>
    			...
    			<td><a href="gm.jsp?id=<%=fl.getId() %><% if(brandid.length()>0) out.print("&bid="+brandid); %>" >修改</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="deleteGdsser('<%= fl.getId() %>')">删除</a>
    			</td>
    			</tr>  
    		  <%}
    	  }%>
      ...
      {%>
    	  还没有系列，<a href="/adming/Gdsser/ag.jsp">马上去添加</a>  
      <%}
   ...
   <%
       ...
       {%><br/><br/>
    	   <div id="updatefl">
    	      <span>修改系列信息</span>
    	      <form id="gdsserupdate" name="gdsserupdate" method="post" action="gm.jsp<% if(brandid.length()>0) out.print("?bid="+brandid); %>" onsubmit="Check();" enctype="multipart/form-data">
