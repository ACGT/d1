...
public static void shipprintOrder(HttpServletResponse response,String odrid,String parternId,String partnerKey){
...
//for(int i=0; i<array.length; i++){
	...
	if(!Tools.isNull(odrm.getOdrmst_goodsodrid())){
		String bigpens=odrm.getOdrmst_ads1();
         ...
         response.sendRedirect("http://www.d1.com.cn/admin/OrderM/odrprintbsht.jsp?odrid="+odrid+"&bigPen="+bigpens);
   		return;
	}
	}catch(Exception ex){
		ex.printStackTrace();
	...
	try{
	 ...
	 InputStream ini = new ByteArrayInputStream(responseString.getBytes("UTF-8"));
	  ...
	  while (resultdetail.hasNext()) {
	   	...
	   	boolean  sendflag=saveShipCode(odrid, "百世汇通", mailNo,bigPen);
		...
		fw.close();
	   	if(sendflag){
	   		...
	   		response.sendRedirect("http://www.d1.com.cn/admin/OrderM/odrprintbsht.jsp?odrid="+odrid+"&bigPen="+bigPen);
	   		return;
	   	}else{
...
shipprintOrder(response,odrid,parternId,partnerKey);
}
