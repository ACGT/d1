...
public static void shipprintOrder(HttpServletResponse response,String odrid,String parternId,String customerId,String clientId){
...
//for(int i=0; i<array.length; i++){
	...
	if(!Tools.isNull(odrm.getOdrmst_goodsodrid())){
		String bigpens=odrm.getOdrmst_ads1();
         ...
         bigpens=URLEncoder.encode(bigpens,"utf-8");
         response.sendRedirect("http://www.d1.com.cn/admin/OrderM/odrprintyt.jsp?odrid="+odrid+"&bigPen="+bigpens);
   		return;
	}
	}catch(Exception ex){
		ex.printStackTrace();
	...
	try{
	 ...
	 InputStream ini = new ByteArrayInputStream(responseString.getBytes("UTF-8"));
	  ...
	  while (resultdetail.hasNext()) {
	   	...
	   	boolean  sendflag=saveShipCode(odrid, "圆通速递", mailNo,bigPen);
		...
		fw.close();
	   	if(sendflag){
	   		bigPen=URLEncoder.encode(bigPen,"utf-8");
	   		response.sendRedirect("http://www.d1.com.cn/admin/OrderM/odrprintyt.jsp?odrid="+odrid+"&bigPen="+bigPen);
	   		return;
	   	}else{
...
public static boolean saveShipCode(String id, String odrmst_d1shipmethod, String odrmst_goodsodrid,String bigpen) {
	try {
...
String parternId = "Y2q75fv1";
String clientId = "K200227788";
String customerId = "K200227788";
...
shipprintOrder(response,odrid,parternId,customerId,clientId);
}
