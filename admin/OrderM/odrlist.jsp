...
public static ArrayList<OrderMain> getOrderMainList(HttpServletRequest request,HttpServletResponse response,String shopCode){
	...
	List<SimpleExpression> listRes = new ArrayList<SimpleExpression>();
	   ...
	   String printpc = request.getParameter("printpc");
	   if(!Tools.isNull(req_odrid)){
		   listRes.add(Restrictions.eq("id", req_odrid));
	   }
	   if(!Tools.isNull(req_goodsodrid)){
	   ...
	   if(!Tools.isNull(wareh)){
		   listRes.add(Restrictions.eq("odrmst_wareh", wareh));
	   }
	   if(!Tools.isNull(printpc)){
		   listRes.add(Restrictions.eq("odrmst_ads2", printpc));
	   }
    ...
    olist.add(Order.desc("odrmst_orderdate"));
    //listRes=null;//测试环境
	List<BaseEntity> list2 = Tools.getManager(OrderMain.class).getList(null, olist, 0, 100);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td>仓库
      <select size="1" name="wareh" id="wareh" style="FONT-FAMILY: 宋体; FONT-SIZE: 9pt">
    ...
    <td>批量打印批次
    <input name="printpc" type="text" size="18" /></td>
