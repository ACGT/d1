<%@ page contentType="text/html; charset=UTF-8"%><%@include file="/html/header.jsp" %><%!
	public static ArrayList<D1ActTb> getacttbList(HttpServletRequest request,HttpServletResponse response){
		...
		List<SimpleExpression> listRes = new ArrayList<SimpleExpression>();
		  String req_name= request.getParameter("req_name");
		   ...
		   String req_shopcode= request.getParameter("req_shopcode");
		   ...
		   if(!Tools.isNull(req_shopcode)){		
			listRes.add(Restrictions.eq("d1acttb_shopcode", req_shopcode));
		   }
		...
		List<BaseEntity> list2 = Tools.getManager(D1ActTb.class).getList(listRes, olist, 0, 5000);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="lin">
 ...
 <%
   ...
   String[] arracttype = new String[]{"满减活动"};  
      String ggURL = Tools.addOrUpdateParameter(request,null,null);
      ArrayList<D1ActTb> list=new ArrayList<D1ActTb>();
      String act=request.getParameter("act");

      list=getacttbList(request,response);
    ...
    if(acttb.getD1acttb_snum3().longValue()>0){ %>
         ...
         <%} %>
           ...
           <div id="actstatus<%=acttb.getId()%>">
           <%=arrstatus[acttb.getD1acttb_status().intValue()] %>
           </div>
           </td>
           <td align="center" width="114">
           <%if(acttb.getD1acttb_status().intValue()==0) {%>
           <div id="actok<%=acttb.getId()%>">
            <input type="submit" name="Submit" onClick="acttbok('<%=acttb.getId() %>','1');" value="通过" />&nbsp;&nbsp;&nbsp;&nbsp;
             <input type="submit" name="Submit" onClick="acttbok('<%=acttb.getId() %>','0');" value="不同意" />
            </div>
            <%} %>
         </td>
         </tr>
