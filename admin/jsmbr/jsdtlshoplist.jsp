...
function update(jsdtl_odrid) {
	...
	for (var i = 0; i < jsdtl_gwjshares.length; i++) {
		if(jsdtl_gwjshares[i].checked) {
			self.location='jsdtlUpdate.jsp?jsdtl_jsmstcode=<%=jsdtl_jsmstcode%>&jsmst_shpcode=<%=jsmst_shpcode%>&jsmst_shpname=<%=jsmst_shpname%>&jsmst_status=<%=jsmst_status%>&jsdtl_odrid='+jsdtl_odrid+'&jsdtl_gwjshare='+jsdtl_gwjshares[i].value+'&href=<%=request.getParameter("href")%>';
		}
    }
}
...
function rtn() {
	self.location='<%=request.getParameter("href")%>';
}
</script>
