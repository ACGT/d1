...
<body>
<center>
<form id="up" name="up" method="post" action="op1.jsp">
<%
...
if(!Tools.isNull(showid)){
	...
	<input type="hidden" id="showid"  name="showid" value="<%=showid%>"/> 
	 订单号：<%=show.getMyshow_odrid() %></td><td>商品编号：<a href="http://www.d1.com.cn/product/<%= show.getMyshow_gdsid()%>" target="_blank"><%=show.getMyshow_gdsid() %></a></td><td>晒单时间：<%=Tools.stockFormatDate(show.getMyshow_createdate())  %></td>
	</tr>
	<tr>
	<td>用户名：<%=show.getMyshow_mbruid() %> <input type="hidden" value="<%=show.getMyshow_mbrid()%>" id="hmbrid" name="mid"/> </td>
  ...
  <option value="30" <%if( show.getMyshow_score()!=null&& show.getMyshow_score()==30) {%> selected="selected"<%} %>>30分</option>
   <option value="50" <%if(show.getMyshow_score()!=null&& show.getMyshow_score()==50) {%> selected="selected"<%} %>>50分</option>
	...
	<tr>
	<td colspan="3"  align="left">内容：<%=show.getMyshow_content() %></td>
	</tr>
	<tr>
  ...
  <option value="2" <%if( show.getMyshow_reasontype()!=null&& show.getMyshow_reasontype()==2) {%> selected="selected"<%} %>>非法晒单</option>
	...
	<td colspan="2"  align="left">审核留言： <textarea id="tcontent" name="tcontent" cols="60" rows="4" style="color:gray;font-size:12px;"></textarea></td></tr>
