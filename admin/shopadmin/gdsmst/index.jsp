...
<%
	...
	if(list!=null && list.size()>0){
		...
		%>
		<table>
		<tr style="background:#EBEBEB; height:30px;"><td>图片</td><td style="width:120px;">商品名称</td><td style="width:90px;">价格</td><td style="width:60px;">规格</td><td style="width:80px;">上下架状态</td><td style="width:80px;">单品赠品</td><td >其他信息</td></tr>
		<%
		...
		%>
			<tr>
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;border-left: 1px solid #C0C0C0;  "><a href="gdsinfo.jsp?gdsid=<%=product.getId()%>" target="_blank"> <img src="<%=img %>" border="0"/></a> <br />
			<span style="color:red;">
			实际库存：<%=product.getGdsmst_stock().intValue() %><br/>
			虚拟库存：<%=product.getGdsmst_virtualstock().intValue() %></span></td>
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;  "><a href="gdsinfo.jsp?gdsid=<%=product.getId()%>" target="_blank"><%=product.getGdsmst_gdsname() %>&nbsp;&nbsp;(<%=product.getId()%>)</a><br />
			<span style="color:red;">
			商户内部编码：<%=product.getGdsmst_shopgoodscode() %>
			</span>
			</td>
			
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;  ">市场价：<%=product.getGdsmst_saleprice().floatValue()%><br/>
			会员价：<%=product.getGdsmst_memberprice().floatValue()%><br/>
			VIP价：<%=product.getGdsmst_vipprice().floatValue()%><br/>
			</td>
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;  ">
			<%if(!Tools.isNull(product.getGdsmst_stdvalue1())){ %><%=product.getGdsmst_stdvalue1()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue2())){ %><%=product.getGdsmst_stdvalue2()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue3())){ %><%=product.getGdsmst_stdvalue3()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue4())){ %><%=product.getGdsmst_stdvalue4()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue5())){ %><%=product.getGdsmst_stdvalue5()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue6())){ %><%=product.getGdsmst_stdvalue6()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue7())){ %><%=product.getGdsmst_stdvalue7()%><br/><%} %>
			<%if(!Tools.isNull(product.getGdsmst_stdvalue8())){ %><%=product.getGdsmst_stdvalue8()%><br/><%} %>
			
			</td>
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;  "><%=flag %></td>
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;  "><%=gift %></td>
			<td style="border-bottom: 1px solid #C0C0C0;border-right: 1px solid #C0C0C0;  ">
