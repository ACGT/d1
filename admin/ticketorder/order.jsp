<%@ page contentType="text/html; charset=UTF-8" import="com.d1.comp.*"%><%@include file="/inc/header.jsp" %><%@include file="/admin/chkrgt.jsp"%><%!
static ArrayList<OrderCache> getOrderCacheList(String odrmst_subad,String cardno,Date s,Date e,String status,String isticket){
	ArrayList<OrderCache> list=new ArrayList<OrderCache>();
	List<SimpleExpression> listRes = new ArrayList<SimpleExpression>();
	if(!Tools.isNull(isticket)){
		if("1".equals(isticket)){
			listRes.add(Restrictions.gt("odrmst_tktvalue",new Double(0) ));
		}else{
			listRes.add(Restrictions.eq("odrmst_tktvalue",new Double(0) ));
		}
		
	}
	if(!Tools.isNull(status)){
		if("0".equals(status)){
			listRes.add(Restrictions.le("odrmst_orderstatus",new Long(status) ));
		}else{
			listRes.add(Restrictions.ge("odrmst_orderstatus",new Long(status) ));
		}
	}
	if(s!=null){
...
public static ArrayList<OrderMain> getOrderMainList(String odrmst_subad,String cardno,Date s,Date e,String status,String isticket){
...
public static ArrayList<OrderRecent> getOrderRecentList(String odrmst_subad,String cardno,Date s,Date e,String status,String isticket){
...
public static ArrayList<OrderBase> getOrderList(String odrmst_subad,String cardno,Date s,Date e,String status,String isticket){
	ArrayList<OrderBase> list=new ArrayList<OrderBase>();
	ArrayList<OrderCache> listcache=getOrderCacheList(odrmst_subad, cardno,s,e,status,isticket);
	if(listcache!=null){
		for(OrderCache ordercache:listcache){
			list.add(ordercache);
		}
	}
	ArrayList<OrderMain> listmain=getOrderMainList(odrmst_subad, cardno,s,e,status,isticket);
	if(listmain!=null){
		for(OrderMain ordermain:listmain){
			list.add(ordermain);
		}
	}
	ArrayList<OrderRecent> listrecent=getOrderRecentList(odrmst_subad, cardno,s,e,status,isticket);
	if(listrecent!=null){
...
<center>
<table>
<tr><td valign="top" style="border:none;"><%@include file="orderleft.jsp"%></td><td style="border:none; width:100%;">
...
if(s==null && e==null && Tools.isNull(orderstatus) && Tools.isNull(isticket) && Tools.isNull(cardno) && Tools.isNull(subad)){
	
	s=new Date(System.currentTimeMillis()-Tools.DAY_MILLIS*15);
	//isticket="wqydyh";
	
}
ArrayList<OrderBase> list= getOrderList(subad,cardno,s,e, orderstatus, isticket);
...
if(list!=null && list.size()>0){
	...
	%>
	<table id="__01"   border="0" cellpadding="0" cellspacing="0" style="font-size:12px; line-height:24px;">
	<tr><td style="border-top:solid 1px #999999;border-left:solid 1px #999999; background:#FF9999;">订单号</td><td style="border-top:solid 1px #999999;background:#FF9999;">订单日期</td><td style="border-top:solid 1px #999999;background:#FF9999;">订单金额</td><td style="border-top:solid 1px #999999;background:#FF9999;">优惠券金额</td><td style="border-top:solid 1px #999999;background:#FF9999;">实际支付金额</td><td style="border-top:solid 1px #999999;background:#FF9999;">订单状态</td><td style="border-top:solid 1px #999999;background:#FF9999;">订单详情</td></tr>
	<%
