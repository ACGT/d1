...
function checkcard(obj){
	...
	if(gdsid.length==0){
		$.alert("参数错误");
		return;
	}else if(gdsid!="01416682" && gdsid!="01413824" && gdsid!="01205265" && gdsid!="01714989" &&gdsid!="01517339" ){
		$.alert("参数错误");
		return;
	}
	if(tailino!=null){
		...
		if(tlen==0){
			...
			if(tailino.indexOf("_9")<0){
				$.alert("您输入错误，请输入台历号_9");	
			}else{
	            $.close();
	            $.inCart1(obj,{ajaxUrl:'/html/zt2012/taili1209/tlincart.jsp?tailino='+tailino,width:400,align:'center'});
			}	
			}
		}
}
