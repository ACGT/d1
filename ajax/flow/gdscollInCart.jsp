...
{
	if(list.get(i)!=null&&list.get(i).getGdscolldetail_gdsflag().longValue()==1)
	{
		Product p=ProductHelper.getById(list.get(i).getGdscolldetail_gdsid());
		if(ProductHelper.isShow(p)){
			plist.add(p);
			gdlist.add(list.get(i));
		}
		else
		{
			str += "，<font color='red'>"+Tools.clearHTML(p.getGdsmst_gdsname())+"</font>";
		}
	}
