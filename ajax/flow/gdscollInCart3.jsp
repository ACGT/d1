 ...
 */
public static int addGdscollToCart(HttpServletRequest request,HttpServletResponse response,HashMap<String,String> pmap,String twohttpurl,int size) {
...
if(Tools.isNull(skuId)){//用户没有传递sku过来。则判断物品是否需要sku，如果不需要，则直接加入购物车，如果需要，则需要弹出sku选择框
	...
	if(isHaveSku == 0){//则直接加入购物车。
		int error = addGdscollToCart(request,response,goodsMap,twohttpurl,goodsMap.size());
