...
PromotionProduct pg = (PromotionProduct)Tools.getManager(PromotionProduct.class).get(id);
if(pg == null){
...
int r=new Random().nextInt(10);
   if(r==0)
	...
	pg.setSpgdsrcm_tghit(pg.getSpgdsrcm_tghit().longValue()+r);
	if(!Tools.getManager(PromotionProduct.class).update(pg,false))
		{
			System.out.print("[团购商品]商品id："+pg.getSpgdsrcm_gdsid()+"更改点击数目出错!");
		}


Product product = ProductHelper.getById(pg.getSpgdsrcm_gdsid());
...
if(CartHelper.hasInCart(product,skuId,request,response) && t==0){
	...
	int s = CartHelper.addTuanProductToCartNew(request,response,id,skuId,Tools.parseInt(count));
