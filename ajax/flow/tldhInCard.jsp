...
if(tuandh==null){
	out.print("{\"code\":1,message:\"该台历券不存在！！\"}");
	return;
}
if(tuandh.getTuandh_status().longValue()==2){
	out.print("{\"code\":1,message:\"该台历券已经兑换过！\"}");
	return;
}
...
if(false && t==0){
	...
	if(hasOne){
		out.print("{\"code\":1,message:\"一个订单最多只能用一个台历券一个此商品！\"}");
		return;
	}
	hasOne = false ;
	if(cartList!=null){
	...
	if(hasOne){
		out.print("{\"code\":1,message:\"此台历券已经放入购物车！\"}");
		return;
	}
	if(tuandh.getTuandh_mid()!=5){
		out.print("{\"code\":1,message:\"此台历券错误！\"}");
		return;
	}
