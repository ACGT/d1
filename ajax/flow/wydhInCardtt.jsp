...
if(false && t==0){
	...
	float ftuanprice=1;
    String strgdsidOne=null;
    String strgdsidTwo=null;
    //Product product = (Product)list.get(0);
    if("01719083".equals(strgdsid)){
    	ftuanprice=5;
    	strgdsidOne=strgdsid;
    }
	...
	int idhcount=getTuandhLength(strgdsidTwo,lUser.getId());
	
	ArrayList<Cart> cartList = CartHelper.getCartItems(request, response);	
	if(cartList!=null){
		for(Cart c123:cartList){
			if(c123.getType().longValue()==13&&c123.getProductId().equals(strgdsidTwo)){
				idhcount++;
			}
		}
	}
	...
	if(idhcount>=2){
		out.print("{\"code\":1,message:\"一个帐号最多只能兑换两个此商品\"}");
		return;
	}
	...
	if(tuandh.getTuandh_mid()!=2 && tuandh.getTuandh_mid()!=4){
