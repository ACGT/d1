			 	    ...
			 	    String id = infoIds.get(i).getKey().toString().trim();
			 	    	 ...
			 	    	 Directory dir=DirectoryHelper.getById(id);
					 	    if(dir!=null){
					 	    	...
					 	    	if(id.equals("022004") || id.equals("022005") || id.equals("022006") || id.equals("022007")){
					 	    		map2.put(id, dir.getId());	
					 	    	}else{
	        ...
	        <span style="color:#fff;font-size:13px; font-weight:bold;">上装</span> </div>
			 ...
			 for (int j = 0; j < infoIds2.size();j++) {
			     ...
			     Directory dir=DirectoryHelper.getById(id);
			 	    ...
			 	    <li <%if(isselect){%>class="current"<%} %>><a href="http://aleeishe.d1.com.cn/ascategorydisplay.htm?productsort=<%=dir.getId()%>"><%=dir.getRakmst_rackname().trim() %></a>
			 	 ...
			 	 for (int k = 0; k < infoIds.size();k++) {
				     ...
				     if(id2.startsWith(dir.getId().trim())){
				    	 ...
				    	 %> 
				    	  <li <%if(isselect2){%>class="selected"<%} %>><a href="http://aleeishe.d1.com.cn/ascategorydisplay.htm?productsort=<%=dir2.getId()%>"><%=dir2.getRakmst_rackname().trim() %></a></li>
				    <%   } }
				      }%>
