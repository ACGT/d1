			...
			if(j==pidlist.length){
				if(Tools.isNull(Tools.getCookie(request,"PINGAN"))){
	    		  ...
	    		  UsrPoint usrpoint = new UsrPoint();
	   		     	usrpoint.setUsrpoint_odrid(orderid);
	   		     	usrpoint.setUsrpoint_mbrid(new Long(lUser.getId()));
	   		     	usrpoint.setUsrpoint_score(new Long(plscore));
	   		     	usrpoint.setUsrpoint_usescore(new Long(plscore));
	   		     	usrpoint.setUsrpoint_type(new Long(2));
	 ...
	 Comment comment=CommentHelper.getById(request.getParameter("commentid"));
				...
				if(comment!=null){
					...
					if(CommentHelper.manager.update(comment, false)){
						...
						usrpoint = UsrPointHelper.getUserPJScore(lUser.getId(),orderid);
						if(usrpoint==null) {
							usrpoint = new UsrPoint();
			   		     	usrpoint.setUsrpoint_odrid(orderid);
			   		     	usrpoint.setUsrpoint_mbrid(new Long(Tools.parseLong(lUser.getId())));
			   		     	usrpoint.setUsrpoint_score(new Long(5));
			   		     	usrpoint.setUsrpoint_usescore(new Long(5));
			   		     	usrpoint.setUsrpoint_type(new Long(3));
			   		     	usrpoint.setUsrpoint_createdate(new Date());
			   		     if(UsrPointHelper.insertUsrPoint(usrpoint) != null) {
		  	    				out.print("1");
		  	    				return;
		  	    			}
						}else {
							int gdscount=getMyCommentlen(new Long(lUser.getId()),orderid); //获得订单商品数
							if(usrpoint.getUsrpoint_score().longValue()<gdscount*5) {
				   		     	usrpoint.setUsrpoint_score(new Long(usrpoint.getUsrpoint_score().longValue()+5));
				   		     	usrpoint.setUsrpoint_usescore(new Long(usrpoint.getUsrpoint_usescore().longValue()+5));
				   		     	usrpoint.setUsrpoint_createdate(new Date());
				   		     	if(Tools.getManager(UsrPoint.class).update(usrpoint, true)) {
				   		     		out.print("1");
		  	    					return;
				   		     	}
							}else {
