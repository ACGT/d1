 ...
 * 调用获取授权session key:http://container.api.taobao.com/container?appkey=21055569<br/>
 ...
 * @throws Exception
 */
public synchronized void orderStateSyncGo(TaobaoClient client,String sessionKey,String from) throws Exception{
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); 
	Date ndate=new Date();
	String endate=sdf.format(Tools.addDate(ndate, -31));
	List<SimpleExpression> clist = new ArrayList<SimpleExpression>();
	clist.add(Restrictions.eq("status", new Long(0)));//状态为0的淘宝订单表示没有同步过发货状态
	clist.add(Restrictions.ge("createdate", sdf.parse(endate)));//状态为0的淘宝订单表示没有同步过发货状态
	if("c".equals(from)){
		clist.add(Restrictions.eq("mbrid", new Long(2055832)));//状态为0的淘宝订单表示没有同步过发货状态
	}else if("b2".equals(from)){
		clist.add(Restrictions.eq("mbrid", new Long(2055841)));//状态为0的淘宝订单表示没有同步过发货状态
	}else{
		clist.add(Restrictions.eq("mbrid", new Long(1544012)));//状态为0的淘宝订单表示没有同步过发货状态
	}
			...
			if(order1!=null){//订单存在
				String shipMethod=order1.getOdrmst_d1shipmethod();
					...
					/*if(shipNumber.startsWith("E")||shipNumber.startsWith("e"))postTaobao="EMS";
						...
						*/
					//if(shipMethod.indexOf("EMS")>=0){ postTaobao="POST";}//POST邮政小包
					if(shipMethod.indexOf("EMS")>=0){ postTaobao="EMS";}
					else if(shipMethod.indexOf("申通")>=0){postTaobao="STO";}
					else if(shipMethod.indexOf("中通")>=0)	{ postTaobao="ZTO";}//顺丰
					else if(shipMethod.indexOf("顺丰")>=0){ postTaobao="SF";}//顺丰
					else if(shipMethod.indexOf("宅急送")>=0){ postTaobao="ZJS";}//宅急送
					else if(shipMethod.indexOf("圆通")>=0) { postTaobao="YTO";}//圆通
					else if(shipMethod.indexOf("韵达")>=0) { postTaobao="YUNDA";}//圆通
					else if(shipMethod.indexOf("全峰")>=0) { postTaobao="QFKD";}//圆通
					else if(shipMethod.indexOf("汇通")>=0) { postTaobao="HTKY";}//圆通
					else if(shipMethod.indexOf("天天")>=0) { postTaobao="TTKDEX";}//圆通
	...
	try{
		...
		if(sku!=null){//商品对应的d1 sku存在才修改库存
			if(hasSku){//读sku表的库存
				...
				if(p!=null&&p.getGdsmst_stocklinkty()!=null&&(p.getGdsmst_stocklinkty().longValue()==3||p.getGdsmst_stocklinkty().longValue()==0)){
					d1stock = 10000l;
				}
...
private void syncTradeList(TaobaoClient client,String sessionKey,List<Trade> list,String from){
	...
	for(Trade t:list){
		...
		//System.out.println(t.getTid()+"========="+t.getBuyerNick());
		try{
			//买家留言、卖家备注要单独获取！！！
			TradeFullinfoGetRequest req=new TradeFullinfoGetRequest();
 ...
 class Thread1 extends Thread
 {
     ...
     private String sessionKey = HttpUtil.getUrlContentByGet("http://www.d1.com.cn/inf/taobao/sk.jsp?get=true", "gbk");
    
     public Thread1(taobaoapi tbapi)
     {
         this.tbapi = tbapi;
     }
     ...
     { 
    	 try{
    	TaobaoClient 	client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", "21055569", "193007595827ff3b6d3a8cac71c80148");
    	 tbapi.tradeSyncGo(client,sessionKey,"tmall");
    	 }catch(Exception ex){
...
if("127.0.0.1".equals(request.getRemoteHost())||"localhost".equals(request.getRemoteHost())){
	...
	taobaoapi tbapi=new taobaoapi();
	TaobaoClient client = null ;
	String sessionKey = null ;
	Date dd=new Date();
	//if (dd.getHours()>=7){
	if("true".equals(request.getParameter("c"))){//C店api调用方法
		System.out.println("淘宝同步C店订单");
		client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", "12483299", "0c4f7116fb2836c0e4d362939b389acc");
		sessionKey = HttpUtil.getUrlContentByGet("http://www.d1.com.cn/inf/taobao/sk2.jsp?get=true", "gbk");
		tbapi.tradeSyncGo(client,sessionKey,"c");//同步交易
	}else if("true".equals(request.getParameter("b2"))){//另外一个b店
		client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", "12483370", "97a3102f358a746e1b50b65870fc2d3e");
		sessionKey = HttpUtil.getUrlContentByGet("http://www.d1.com.cn/inf/taobao/sk3.jsp?get=true", "gbk");
		tbapi.tradeSyncGo(client,sessionKey,"b2");//同步交易
	}else{//商城api调用
		System.out.println("淘宝同步官方店订单");
	     Thread t1 = new Thread1(tbapi);
	      t1.start();
	      System.out.println("淘宝:"+new Date());
		/*
		client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", "21055569", "193007595827ff3b6d3a8cac71c80148");
		
		sessionKey = HttpUtil.getUrlContentByGet("http://www.d1.com.cn/inf/taobao/sk.jsp?get=true", "gbk");
		System.out.println("淘宝:"+new Date());
		tradeSyncGo(client,sessionKey,"tmall");//同步交易*/
	      client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", "21055569", "193007595827ff3b6d3a8cac71c80148");
	  	sessionKey = HttpUtil.getUrlContentByGet("http://www.d1.com.cn/inf/taobao/sk.jsp?get=true", "gbk");
	}
	int currentMinute = java.util.Calendar.getInstance().get(java.util.Calendar.MINUTE) ;
	if (dd.getHours()>=13&&dd.getHours()<23){
	 //if(!"true".equals(request.getParameter("c"))){//C店api调用方法
	    if(dd.getHours()%2==0&&currentMinute==0){//库存1小时同步一次
		   System.out.println("淘宝库存更新:"+new Date());
		  // tbapi.stockSyncGo(client,sessionKey);
		   //tbapi.stockSyncGo1(client,sessionKey);
	   }
	 //}
	 if(currentMinute%32==0){
	    if("true".equals(request.getParameter("c"))){//C店api调用方法
	    	tbapi.orderStateSyncGo(client,sessionKey,"c");//同步订单状态
	    }else if("true".equals(request.getParameter("b2"))){//另外一个b店
	    	tbapi.orderStateSyncGo(client,sessionKey,"b2");//同步订单状态
	    }else{
	    	tbapi.orderStateSyncGo(client,sessionKey,"tmall");//同步订单状态
	    }
	    tbapi.syncSellerMemo(client,sessionKey);//同步卖家留言
	 }
	}
