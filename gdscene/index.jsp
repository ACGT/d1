...
.clear {clear:both;font-size:1px;line-height:0;height:0px;*zoom:1;}
    ...
    .pricegd{background-color: #fff;width:160px; filter: alpha(opacity=80);-moz-opacity: 0.8;opacity: 0.8; height:260px;position:absolute; bottom:50px; left:120px;  z-index:99; border:solid 1px #ccc;   }
    ...
    .pricegds{background-color: #fff; filter: alpha(opacity=90);-moz-opacity: 0.9;opacity: 0.9;width:160px; position:absolute; bottom:27px; left:430px;  z-index:99; border:solid 1px #fff;   }
    ...
    .otherpjfs{  background-color: #000; height:auto;position:absolute; border:solid 1px #fff; margin-left:520px; margin-top:-225px; +margin-top:275px; +margin-left:165px; z-index:10000;  display:none;padding-left:10px;}
     ...
     <div class="gdscene_center">
		    	   ...
		    	   <%
		    	    ...
		    	    <div style="padding-top:0px; border-top:solid 1px #ccc;margin:0px auto;overflow:hidden; text-align:left; line-height:20px; padding-top:5px; padding-left:11px;color:#000;">
	                                ...
	                                <strike>总价：￥<font id="memberP1"><%=(int)sum %></font></strike><br/>组合价：<b>￥<font style='color:#be0000; font-size:14px;' id="pktP1"><%= (int)zhprice %></font></b><br/>共优惠：￥<font id="cheap1"><%= (int)(sum-zhprice) %></font>
	                                 ...
	                                 <span style="margin-top:5px; padding-left:1px; margin-bottom:10px; height:40px; ">
	                                 <font style="font-size:16px; color:#f00;">搭配购买立享95折！</font><br/>
      ...
      %>     
       ...
       <%
           ...
           { int count=0;%>
        	    <div class="gdscsne_dp" style="background:#<% if(gdsscene.getGdsscene_color()!=null&&gdsscene.getGdsscene_color().length()>0) out.print(gdsscene.getGdsscene_color().toString()); else out.print("fff"); %>">
                <ul>
        	  <%         	    
        	   for(int i=gdsscene.getGdsscene_logo().longValue()==0?0:1;i<gdscolllist.size();i++)
        	   {
        		   Gdscoll gds=gdscolllist.get(i);
        		   ...
        		   { 
        			...
        			<div style="position:relative;">
        			 <div style=" width:316px; height:380px; overflow:hidden; background:url('http://images1.d1.com.cn<%= gdsscene.getGdsscene_imgbg()%>') repeat-x; ">
        			    <a href="http://www.d1.com.cn/gdscoll/index.jsp?id=<%= gds.getId() %>&sf=<%= gdsscene.getId() %>" target="_blank"><img src="http://images1.d1.com.cn<%= gds.getGdscoll_brandimg() %>" style="left:-60px; position:absolute; margin-top:22px; z-index:22;" /></a>
        			  ...
        			  <%  
        			      ArrayList<Gdscolldetail> nogdlist=getOtherGdscoll(gds.getId());
        			      ...
        			      if(gdlist!=null&&gdlist.size()>0)
        			      {
        			        ...
        			        %>
        			    	   <div class="pricegd">
        			    	   <div style="width:98%; height:113px; margin:0px auto; overflow:hidden; padding-top:4px;">
        			    	   ...
        			    	   <%
        			    	       ...
        			    	       for(Gdscolldetail gd:gdlist)
        			    	       {
        			    	    	   ...
        			    	    	   if(p!=null&&p.getGdsmst_ifhavegds().longValue()==0&&p.getGdsmst_validflag().longValue()==1&&gd.getGdscolldetail_gdsflag()!=null&&gd.getGdscolldetail_gdsflag().longValue()==1&&ProductStockHelper.canBuy(p))
        			    	    	   {
        			    	    		   ...
        			    	    		   zhprice+=Tools.getFloat((int)(p.getGdsmst_memberprice().floatValue()*zk), 2);
        			    	    		   %>
        			    	    		   <span><a href="http://www.d1.com.cn/product/<%= p.getId() %>" target="_blank" ><%= gd.getGdscolldetail_title() %></a>:<font class="font1">￥<%= (int)Math.ceil(p.getGdsmst_memberprice().floatValue())%></font>&nbsp;&nbsp;&nbsp;&nbsp;
        			    	    		   <a href="javascript:void(0)" attr="<%= p.getId() %>" onclick="AddGdscollInCart(this);" ><img src="http://images.d1.com.cn/images2012/index2012/buy.gif" style="vertical-align:text-bottom;"/></a>
        			    	    	       </span>
        			    	   ...
        			    	   <%
        			    	       if(nogdlist!=null&&nogdlist.size()>0)
        			    	       {%>
        			    	    	   <span style="height:28px; line-height:28px; background:#000; color:#fff; cursor:hand;filter: alpha(opacity=80);-moz-opacity: 0.8;opacity: 0.8; " onmouseover="gdscollover('<%= gds.getId() %>')" onmouseout="gdscollout('<%= gds.getId() %>')">+&nbsp;&nbsp;&nbsp;选择其他配件>></span>
        			    	       <%}
        			    	   ...
        			    	   %>
        			    	    ...
        			    	    <div style="width:95%; padding-top:0px; border-top:solid 1px #ccc;margin:0px auto;overflow:hidden; padding-left:11px; line-height:20px; padding-top:5px; color:#000;">
        			    	           共<em id="amount<%=count%>"><%= counts%></em>件 <br/>
                                <strike>总价：￥<font id="memberP<%= count%>"><%=(int)sum %></font><br/></strike>组合价：<font style='color:#be0000; font-size:14px;'><b>￥<font id="pktP<%= count%>"><%= (int)zhprice %></font></b></font><br/>共优惠：￥<font id="cheap<%= count%>"><%= (int)(sum-zhprice) %></font>
                              
                                 <span style="margin-top:5px;padding-left:1px; height:50px; ">
                                 <font style="font-size:16px; color:#f00;">搭配购买立享95折！</font><br/>
                <a href="javascript:void(0)" code="<%= gds.getId() %>" onclick="check_gdscoll1(this,'<%= count %>')" style="color:#fff; font-size:14px;"><img src="http://images.d1.com.cn/images2012/index2012/zhgm.png"/></a></span>
                               </div>
        			    	   </div>
        			      <%}
        			  %>
        			  ...
        			  <%
	        			  ...
	        			  {
	        			   %>
	        				  <div class="<% if(count%3==0) out.print("otherpjf"); else out.print("otherpjf"); %>" id="other_<%= gds.getId() %>" onmouseover="gdscollover(<%= gds.getId() %>)" onmouseout="gdscollout(<%= gds.getId() %>)" >
        			            <table style="text-align:center; background:#fff;  padding-right:10px;">
	        				 <% for(Gdscolldetail gdetail:nogdlist)
	        				  {
	        					  if(gdetail!=null)
	        					    	  ...
	        					    	  Product pro=ProductHelper.getById(gdetail.getGdscolldetail_gdsid());
	        					    	  {
	        					    		 if(pro!=null&&pro.getGdsmst_ifhavegds().longValue()==0&&pro.getGdsmst_validflag().longValue()==1&&ProductStockHelper.canBuy(pro))
	        					    		 {
	        					    		       ...
	        					    		       else
	        					    		       {
	        					    		           ...
	        					    		           <tr><td  width="30" height="60">
	        					    		           <input type="checkbox" name="chk_<%=count %>" value="<%= gdetail.getGdscolldetail_gdsid() %>" onClick="selectInitdp('<%=Tools.getFormatMoney(pro.getGdsmst_memberprice())%>',this.checked,<%= count%>)" /></td><td width="60"><img src="<%= imgurl %>" width="50" height="50"/></td>
        			                                   <td style="text-align:left;"><a href="/product/<%= gdetail.getGdscolldetail_gdsid() %>" target="_blank"><%= gdetail.getGdscolldetail_title() %></a><br/>
        			                                   <font class="font1">￥<%=  Tools.getFormatMoney(pro.getGdsmst_memberprice().floatValue()) %></font>&nbsp;&nbsp;&nbsp;&nbsp;       			                                     	      
        			                                   </td>
        			                                   <td>  <a href="javascript:void(0)" attr="<%= pro.getId() %>" onclick="AddGdscollInCart(this);" ><img src="http://images.d1.com.cn/images2012/index2012/buy.gif" style="vertical-align:text-bottom;"/></a>
        			    	</td>
