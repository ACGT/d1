...
<div class="lf">
	<p style="z-index:999; position:relative; "><a href="<%=ProductHelper.getProductUrl(product) %>" target="_blank" >
           								<img src="<%= theimgurl%>" width="200" height="200"  alt="<%= imgalt %>"/>
		...
		<%}else{
			%>
		<span style="position:absolute; width:45px; height:56px; dislay:block; background:url('http://images.d1.com.cn/zt2012/20121113flcx/ann.png'); left:8px; top:-8px; z-index:5000;"></span>
			<%}
		%>
		...
		<div  style="font-size:12px;padding-left:5px;padding-right:5px;padding-top:3px; padding-bottom:0px;height:38px;">
		 <table style="height:38px;">
			              <tr>
			              <td valign="middle">
		<%=StringUtils.getCnSubstring(Tools.clearHTML(pProduct.getSpgdsrcm_gdsname()),0,56)%>
		</td></tr></table>
		 </div>
