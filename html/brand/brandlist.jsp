	...
	return;
		...
		Counter counter=(Counter)Tools.getManager(Counter.class).get(id);
				if(counter!=null){
					...
					header_sex_type=counter.getCounter_sex().toString();
...
<div style="width:980px;margin:0 auto;overflow:hidden;_zoom:1;">
  ...
  <%
     ...
     String memberprice=request.getParameter("productprice");
     String brand=request.getParameter("productbrand");
     String productsex=request.getParameter("productsex");
  ...
  %>
    <!-- 长图显示 -->
   <% if(request.getParameter("f")=="hd") 
      {
           if(productsort=="014"&&(id=="10"||id=="21"||id=="22"||id=="24"))
           {
        	   if(((brand=="lancome" || brand=="倩碧") && productsex=="1") || brand=="兰芝" || brand=="雅顿")
        	   {
        		   %>
     <center>
         <a href="http://www.d1.com.cn/html/zt2010/03057zhounian/" target="_blank">
         <img src="http://images.d1.com.cn/homeimg2010/indexbanner_3.gif" width="980" height="110" border="0" />
         </a>
     </center>
        		   <%
        	   }
      ...
      if(counter_titlepic.length()!=0)
      {
    	  out.print(counter_titlepic);
      }
      else if(counter_pictong.length()==0||Tools.isNull(counter_pictong))
      {%>
    	<table width="960" border="0" cellpadding="0" cellspacing="0" align=center>
	<tr>
	    <td>
	        <%if(counter_pic.length()!=0)
	          {%>
			<a href="/html/brand/brand<%=id%>.htm">
			<img src="<%=counter_pic%>" alt="" border=0>
			</a>
			<% } %>
		</td>
		<td valign=top>
      ...
      {
    	  out.print("<img src=\""+counter_pictong+"\" border=0/>");
      }
	...
	<%						
	String urlsql="brandlist.jsp?id="+id;
                    	...
                    	if(gdsmst_ifhavegds.equals("2") || gdsmst_ifhavegds.equals("3") || gdsmst_ifhavegds.equals("1")){
                    		if(gdsmst_ifhavegds.equals("1")){
                    			if( product.getGdsmst_ifhavedate().after(new Date(System.currentTimeMillis()-24*3600*1000l))){
                    				int   ddddd   =   (int)((product.getGdsmst_ifhavedate().getTime()   -   new Date().getTime())   /   (1000   *   60   *   60   *   24))+1;
                    				errmst_intro="此商品暂时缺货，预计"+ddddd+"天内到货。";
                    			}else{
...
<%! 
    private String GetUrl(String filed,String value)
	{
	  String result="";
	  if(value.length()>0)
	  {
