...
public static ArrayList<FriendLink> getFriendLinkList(String counterid)
{
	
	ArrayList<FriendLink> list=new ArrayList<FriendLink>();
	List<SimpleExpression> clist = new ArrayList<SimpleExpression>();
	clist.add(Restrictions.eq("friendlink_type", new Long(counterid)));
	List<BaseEntity> blist=Tools.getManager(FriendLink.class).getList(clist, null, 0, 80);
...
.newbanner1120 {position: fixed;z-index: 20000;top: 0;background: white;text-align: left;background:url('http://images.d1.com.cn/images2010/listtypebg.gif');}

#brandlink .text{width:980px;margin:auto;padding:20px;line-height:14px;position:relative;}
#brandlink .text span{float:left;color:#a3a3a3;}
#brandlink .text p{float:left;width:880px;height:14px;overflow:hidden;}
#brandlink .text p.hg_auto{height:auto;}
#brandlink .text s{position:absolute;right:10px;top:20px;cursor:pointer;width:0;height:0;font-size:0;border:#ccc 8px;border-style:solid dashed dashed dashed;border-color:#ccc transparent transparent transparent;}
#brandlink .text s.current{top:12px;border-style:dashed dashed solid dashed;border-color:transparent transparent #ccc transparent;}
#brandlink .text p a{float:left;padding:0 6px;margin-left:-1px;margin-bottom:5px;border-left:#ccc solid 1px;color:#ccc;}

</style>
<script type="text/javascript">
function get1(tcode,code){
	if(tcode.length>0){
			$.post("/ajax/html/getBrandInfo.jsp",{"tcode":tcode,"code":code},function(data){
				if(data.success){
				$('#divbrand').html(data.content);
...
<%
 if("33".equals(id)){
	 ...
	 response.sendRedirect("http://www.d1.com.cn/result.jsp?productsort=014&productname=%E5%A4%A7%E5%8D%AB%E6%9D%9C%E5%A4%AB");
	 return;
 }else if("45".equals(id)){
	 response.sendRedirect("http://www.d1.com.cn/html/special/zippo/zippo.jsp");
	 return;
 }
...
<%}else{
	...
	if("13".equals(id) || "110".equals(id)){//佰草集或欧莱雅
		if(session.getAttribute("first_referer_url2")!=null){
			...
			}else if("111".equals(id)){
				if(first_referer_url.indexOf("玫琳凯")>0 || first_referer_url2.indexOf("玫琳凯")>0){
				isfrom=3;
				}
			}else if("144".equals(id)){
				if(first_referer_url.indexOf("玉兰油")>0 || first_referer_url2.indexOf("玉兰油")>0){
				isfrom=4;
				}
			}
		}
...
<table width="960" border="0" cellpadding="0" cellspacing="0">
	...
	<div id="rank">
		<ul>
			...
			<ul id="rankl<%=no%>">
			<%
			ArrayList<Product> plistbrand=ProductHelper. getProductByCodeAndBName(item2.getCounterdtl_code(),item2.getCounterdtl_imgurl(),8);
			if(plistbrand!=null){
				int i=1;
				for(Product p:plistbrand){
...
document.getElementById('ran'+wh).className='c';
	  ...
	  <font color="#cc0000">品牌</font>故事</td>
		...
		<td width=750 align=right valign=top>
				 ...
				 <table id="nav" width="750" height="31" border="0" align="center" class="lincss"  background="http://images.d1.com.cn/images2010/listtypebg.gif"cellpadding="0" cellspacing="0">
			      <tr>
				...
				//out.print("<script>alert('"+sequence+"')</script>");
					...
					Date discountenddate= 	sdf.parse(gdsmst_discountenddate);
					 String img=product.getGdsmst_imgurl();
					 if(!Tools.isNull(img)){
                   ...
                   <% if(product.getGdsmst_addshipfee().intValue()!=0){
                    ...
                    <% }
                    if(product.getGdsmst_eyuan()!=null&&product.getGdsmst_eyuan().floatValue()>0f){
                    	 %>  
                    ...
                    <%	 if(msflag){
								%>	
								<font color=#4D4D4D  style="font-size:9pt"><strike>原价: <font style="font-size:10.5pt"><b><%=mprice %></b></font>元</strike></font><br/>
		  						 <%} %>
		  						 <br /><span style="font-size:18px; font-weight:800; color:red">￥<%=gprice%></span><span style="text-decoration: line-through; padding-left:16px; font-size:14px; color:#4D4D4D">￥<%=sprice%></span>
							</div>
                    </a>
                    <%
    ...
    <p>      
            <%
    	    	...
    	    	for(FriendLink fl:list)
    	    	{
                   if(fl!=null)
                   {
                	   sb.append("<a href=\"").append(fl.getFriendlink_website().trim()).append("\" target=\"_blank\"");
...
$(document).ready(function() {
	...
	if(i>=m)
	{
		...
		//友情链接
		$("#brandlink .text s").toggle(function(){
			$("#brandlink .text p").addClass("hg_auto");
			$(this).addClass("current");
		},function(){
			$("#brandlink .text p").removeClass("hg_auto");
