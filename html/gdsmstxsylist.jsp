...
function CheckForm(obj){
    ...
    //alert($(obj).attr("code"));
   $.inCart1(obj,{ajaxUrl:'/ajax/flow/listXsyInCartNew.jsp',width:600,align:'center'},{'code':$(obj).attr("code")});
}

function checknum(obj){
	 var strMaxCount = Number($.trim($('#hdnMaxCount').val()));
	 var num=Number($.trim($(obj).val()));
	 if(num>strMaxCount){
		 $.alert('最多能选择'+ strMaxCount + '件商品!');
		 $(obj).val(1)
	 }
...
<body>
  <div id="wrapper">
	...
	%>
	    		   ...
	    		   if(ProductHelper.isNormal(p))
	    		   {
	    			   ...
	    			   %>
	    			     <div style=" width:204px; overflow:hidden; float:left; margin-right:0px;">
	     					...
	     					<input type="checkbox"  name="gdsid" value="<%= gdsid %>"/><font color="#ff0000"><b>选择此商品</b></font>
	     					<br/>
	     					<p style="padding-bottom:5px;">购买数量：<input type="text" value="1" style="width:30px;" id="<%=p.getId() %>_num"  onblur="checknum(this)" onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onafterpaste="this.value=this.value.replace(/[^\d]/g,'') "/></p>
	     					
		 					<a href="http://www.d1.com.cn/product/<%= gdsid %>" target="_blank">
		 					<img src="<%= imgurl %>" width="200" height="200"/></a><br/>
		 					</div>
	    			   ...
	    			   else
	    			   {
	    				   ...
	    				   <div style=" width:204px; overflow:hidden; float:left; margin-right:54px;">
	     					...
	     					<input type="checkbox"  name="gdsid" value="<%= gdsid %>"/><font color="#ff0000"><b>选择此商品</b></font>
	     					<br/>
	     					<p style="padding-bottom:5px;">购买数量：<input type="text" value="1" id="<%=p.getId() %>_num" style="width:30px;" onblur="checknum(this)" onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onafterpaste="this.value=this.value.replace(/[^\d]/g,'') "/></p>
       ...
       <input type="image" name="btnAddToCart" src="http://images.d1.com.cn/images2013/act/flowadd.png" onclick="CheckForm(this);" code="<%=code %>" id="btnAddToCart">
        </div>
	 </div>
