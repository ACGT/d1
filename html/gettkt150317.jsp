...
Date mbrDate=null;
try{
	 dStartDate =fmt.parse("2015-11-14");
...
try{
	endDate=fmt2.parse("2015-11-13 23:59:59");
}
catch(Exception ex){
	 ex.printStackTrace();
}
if(tktcard == null) {
	//for(int i=0;i<5;i++){
		...
		tktmst1.setTktmst_value(new Float(20));
		...
		tktmst1.setTktmst_rackcode("000");
		tktmst1.setTktmst_gdsvalue(new Float(100));
		tktmst1.setTktmst_payid(new Long(-1));
		tktmst1.setTktmst_cardno(strtkt);
		tktmst1.setTktmst_ifcrd(new Long(0));
		tktmst1.setTktmst_shopcodes("11111111");
		tktmst1.setTktmst_memo("活动领券--邹！！");
		...
		tktmst3.setTktmst_gdsvalue(new Float(200));
		...
		tktmst4.setTktmst_gdsvalue(new Float(200));
	...
	Tools.outJs(out,"恭喜你，获得2张满200元减30元购物券全场通用、2张满100元减20元购物券全场通用（特别标注不让用券的特价商品除外）购物券有效期是2015年11月13日",gourl);
