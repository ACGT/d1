...
if(request.getAttribute("subad")!=null){
	subad=(String)request.getAttribute("subad");
}
...
if(gdslist!=null){
	...
	if(productlist!=null){
		%>
		 <div style="background-color:#CCCCCC; width:750px;padding-bottom:18px;"> 
	...
	<%	
		for(Product product:productlist){
			...
			if(pproductlist!=null && directory!=null){
			 ...
			 if (pProduct.getSpgdsrcm_gdsname()!=null){
				 imgalt=StringUtils.replaceHtml(pProduct.getSpgdsrcm_gdsname());
			 }
...
<td width=15>&nbsp;</td>
	 <td width="210" align="center" valign=top>
		...
		<a  href="<%if (pProduct.getSpgdsrcm_otherlink().trim().length()==0){%>http://www.d1.com.cn/html/index99.jsp?id=p130409xjqhd&md=d1_1111&url=http://www.d1.com.cn/product/<%=product.getId()%>
		<%}else{%>
		<%=StringUtils.encodeUrl(pProduct.getSpgdsrcm_otherlink().trim())%>
		<%}%>" target=_blank style='text-decoration:none'><font style="font-size:10pt;color:#000"><%=Tools.substring(imgalt,25)%></font><br><br>
		</a>
		<%
		 if(booldx && dxprice>0){
			 %> 
			  <span style="font-size:12px;font-weight:bold;color:#666666;"><strike>原售价:￥<%=strmprice%></strike></span>
		<span style="font-size:15px;font-weight:bold;color:#ff0000;"><%=str %>:￥<%=ProductGroupHelper.getRoundPrice(dxprice)%>&nbsp;</span><br>
		...
		<div align="right" style="height:40px">
			<a href="http://www.d1.com.cn/html/index99.jsp?id=p130409xjqhd&md=d1_1111&url=http://www.d1.com.cn/product/<%=product.getId()%>" target="_blank"><img src="http://images.d1.com.cn/images2011/sales/tm004.gif" border="0" /></a>
		</div>
		</td></tr>
		</table>
		</center>
