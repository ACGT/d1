...
top: 0;
background: white;
background: url(http://images.d1.com.cn/xinpin/bjc.gif);
text-align: left;
height: 39px;
width: 980px;
...
#nav_head {
	background-image: url(http://images.d1.com.cn/xinpin/head1.jpg);
	background-repeat: no-repeat;
	width: 980px;
	height: 92px;
}
...
.nav121203 .bt_qnav { float:right; }	
.nav121203 .bt_qnav a{ width:31px; height:29px; line-height:39px;display:block;padding:9px 2px 0 0;}
.nav121203 .c{ float:left;margin:0;padding:0}

.nav121203 li { float:left; list-style:none; }
.nav121203 li .v a{
	width:65px;
	height:33px;
	display:block;
...
.nav121203 li .v a:hover,#nav li .v .sele{
	color:#116406;
	line-height:33px;
	font-size:14px;
...
.newbanner1120 .bt_qnav { float:right; }	
.newbanner1120 .bt_qnav a{ width:31px; height:29px; line-height:39px;display:block;padding:9px 2px 0 0;}
.newbanner1120 .c{ float:left;margin:0;padding:0}

.newbanner1120 li { float:left; list-style:none; }
.newbanner1120 li{
	text-align:center;
	font-size:12px;
	background-image: url(http://images.d1.com.cn/xinpin/fenge.gif);
...
.newbanner1120 li .v a:hover,#nav li .v .sele{
	color:#116406;
	line-height:33px;
	font-size:14px;
	background-image: url(http://images.d1.com.cn/xinpin/fn.gif);
...
<div id="wrapper">
       ...
       if(rackcode.length()>0)
       {
    	   ...
    	   if(rackcode.indexOf(",")>0)
    	   {
    		   ...
    		   if(lall.size()<50)
    		   {
    			   ...
    			   for(int i=0;i<rackcodes.length;i++)
        		   {
        			   ArrayList<Product> subl=ProductHelper.getProductListByRCode(rackcodes[i], 25);
        			   if(subl!=null&&subl.size()>0)
        			   {
        				   lall.addAll(subl);
        			   }
    	   ...
    	   else
    	   {
    		   ...
    		   if(sl.size()<50)
    		   {
    			   lall=ProductHelper.getProductListByRCodeSub(rackcode, 50);
    		   }
    		   else
    		   {
    			   lall=sl;
   ...
   %>
     ...
     <div class="center">
			     <DIV id=nav_wrap>
					...
					<UL class=c>
					  ...
					  <LI><SPAN class=v ><A href="index.jsp?productsort=020"  <%if(rackcode=="020"){%> class="sele" <%}%>>女装</A></SPAN> </LI>
					  <LI><SPAN class=v ><A href="index.jsp?productsort=021"  <%if(rackcode=="021"){%> class="sele" <%}%>>女鞋</A></SPAN> </LI>
					  ...
					  <LI><SPAN class=v  ><A href="index.jsp?productsort=030"<%if(rackcode=="030"){%> class="sele" <%}%>>男装</A></SPAN> </LI>
					  <LI><SPAN class=v ><A href="index.jsp?productsort=031"  <%if(rackcode=="031"){%> class="sele" <%}%>>男鞋</A></SPAN> </LI>
					  <LI><SPAN class=v ><A href="index.jsp?productsort=015002004"  <%if(rackcode=="015002004"){%> class="sele" <%}%>>手表</A></SPAN> </LI>
					  <LI><SPAN class=v ><A href="index.jsp?productsort=033" <%if(rackcode=="033"){%> class="sele" <%}%>>皮具</A></SPAN> </LI>
					  ...
					  <LI><span class=x ><a href="index.jsp"><img src="http://images.d1.com.cn/xinpin/all.jpg" border="0" /></a></span></LI>
					  </UL>
					<DIV class=r></DIV>
					</DIV><!--nav-->
			</DIV><!--nav_wrap-->
     ...
     <center>
	     <div id="newgdslist" style="margin:0px auto; width:980px;overflow:hidden;">
	        	    ...
	        	    //{
                        ...
                        %>
                              <div style=" width:204px; overflow:hidden; float:left;margin-right:0px;">
							     <div style="text-align:center;  overflow:hidden; font-size:13px; ">
								 <a href="/product/<%= gdsid %>" target="_blank" title="<% 

StringUtils.replaceHtml(p.getGdsmst_gdsname()); %>">
								 <img src="<%= imgurl %>" width="200" height="200" style="border:solid 1px #ccc;"/>
								 </a><br/>
								 <!-- 显示特价图标 -->
								 <% Date d=new Date();
								    ...
								    { %>
								 <div style="position:absolute;z-index:999;  margin-top:-206px; margin-left:-4px; _margin-top:-225px;  _margin-left:-104px; +margin-top:-225px; +margin-left:-104px; margin-top:-210px\0; margin-left:-2px\0;"><img src="http://images.d1.com.cn/images2010/tejia2.gif" /> </div>	
								 <% } %>
								 ...
								 <!-- 是否添加运费 -->
								    ...
								    <span style="font-size:16px; color:#B50126; font-weight:bold; font-family:'微软雅黑'">￥<%= p.getGdsmst_memberprice()%></span>
								    <span style="text-decoration: line-through; padding-left:16px; font-size:14px; color:#999">￥<%= p.getGdsmst_saleprice() %></span>
								   <!-- 判断是否是平安用户 -->
