...
Date mbrDate=null;
try{
	 dStartDate =fmt.parse("2015-04-01");
...
if(lUser==null) {
	...
	if(httpurl.startsWith("m.d1.cn")){
		response.sendRedirect("http://m.d1.cn/wap/login.html?url=/html/newyear2015hb.jsp");
		return;
	}else{
...
try{
	endDate=fmt2.parse("2015-03-31 23:59:59");
}
catch(Exception ex){
	 ex.printStackTrace();
}
if(tktcard == null) {
	//for(int i=0;i<5;i++){
		...
		tktmst.setTktmst_gdsvalue(new Float(198));
		...
		tktmst2.setTktmst_gdsvalue(new Float(100));
		...
		tktmst3.setTktmst_rackcode("030");
		...
		tktmst3.setTktmst_memo("新年红包活动--邹！");
		Tools.getManager(Ticket.class).create(tktmst3);
		
		Ticket tktmst4=new Ticket();
		tktmst4.setTktmst_value(new Float(60));
		tktmst4.setTktmst_type("005009");
		tktmst4.setTktmst_mbrid(Tools.parseLong(lUser.getId()));
		...
		tktmst4.setTktmst_rackcode("015");
		tktmst4.setTktmst_gdsvalue(new Float(100));
		tktmst4.setTktmst_payid(new Long(-1));
		tktmst4.setTktmst_cardno(strtkt);
		tktmst4.setTktmst_ifcrd(new Long(0));
		tktmst4.setTktmst_shopcodes("00000000");
		tktmst4.setTktmst_memo("新年红包活动--邹！");
	...
	Tools.outJs(out,"恭喜你，获得1张满198元减20元购物券全场通用、1张满100元减60元购物券自营男装可用、1张满100元减60元购物券自营女装可用 、1张满100元减60元购物券自营配饰可用，（特别标注不让用券的特价商品除外）购物券有效期是2015年2月28日",gourl);
