      ...
      %>
	             ...
	             <td>
                    ...
                    <%  if(gdscolllist.size()>1){ %>
                      <div class="pre" id="pre2012<%=count%>" attr="<%= gdsid %>"><img src="http://images.d1.com.cn/images2012/index2012/left.png"/></div>
                      <div class="next" id="next2012<%=count%>" attr="<%= gdsid %>"><img src="http://images.d1.com.cn/images2012/index2012/right.png"/></div>
			          <%} %>
			    <div class="clear"></div>
			    <!-- 背景图片 -->
			    <%
			  ...
			  <%  for(Gdscoll gdscoll:gdscolllist)
				  {
					  ...
					  { counts++;%>
                        ...
                        <a href="http://www.d1.com.cn/gdscoll/index.jsp?id=<%= gdscoll.getId() %>" target="_blank" style="display:block; width:246px; height:350px;" title="<%= Tools.clearHTML("【"+gdscoll.getGdscoll_title()+"】"+gdscoll.getGdscoll_tail()) %>&nbsp;点击查看搭配超清晰大图>>">
			            	...
			            	<% for(Gdscolldetail gd:egdlist)
			            	   {
			            		   ...
			            		   <div>
			            			   <a href="http://www.d1.com.cn/product/<%= gd.getGdscolldetail_gdsid() %>" target="_blank" style="background:#fff;" title="<%= Tools.clearHTML(p.getGdsmst_gdsname()) %>"><img src="<%= imgurl %>" width="75" height="75"/></a>
			            			   <div>
			            			    <input type="checkbox" name="chk_<%=gdsid %>_<%=count%>_<%=counts %>" <% if(gd.getGdscolldetail_gdsflag()!=null&&gd.getGdscolldetail_gdsflag().longValue()==1) out.print("checked"); %>  value="<%= gd.getGdscolldetail_gdsid() %>" attr="<%= gd.getGdscolldetail_gdsid().trim() %>"  onClick="selectdp(<%= counts %>,'<%=gdsid %>',<%= count %>)" m="<%=p.getGdsmst_memberprice()%>">&nbsp;￥<%= p.getGdsmst_memberprice() %></input>
			            		       </div>
			            		  </div>
			            		   <%}
			            		   }
