...
ArrayList<Product> productlist=getProduct(code,100);
	...
	<%} 
	%>
	<td style="background:url('http://images.d1.com.cn/zt2012/20120928sytl/sybj_05.jpg') no-repeat;" width="302" height="404" align="center">
	<table height="404">
	<tr><td valign="top" height="302"><a href="<%if (pProduct.getSpgdsrcm_otherlink().trim().length()==0){%>/product/<%=product.getId()%><%}else{%><%=pProduct.getSpgdsrcm_otherlink().trim()%><%}%>" target=_blank style='text-decoration:none' title="<%=pProduct.getSpgdsrcm_gdsname()%>">
		...
		<tr><td height="13">&nbsp;</td>
		<tr><td width="300px" height="26px" align="center"  style="color:#ccc;font-size:14px;font-weight:bold;"><%=StringUtils.getCnSubstring(Tools.clearHTML(pProduct.getSpgdsrcm_gdsname()),0,36) %></td></tr>
		<tr><td align="center" valign="top" style="color:#ccc;">
		<table width="302">
		<tr><td width="72" align="center" valign="middle"><span style="font-size:12px;">原价:￥<%=Tools.getFormatMoney(product.getGdsmst_oldmemberprice()) %></span></td>
		<td><span style="font-size:30px;font-weight:bold;color:white;font-family:'微软雅黑';color:#ED7E2C">半价:￥<%=Tools.getFormatMoney(product.getGdsmst_memberprice()) %></span></td>
		<td width="60">
		<%
		if(Tools.longValue(product.getGdsmst_validflag())!=1 || Tools.longValue(product.getGdsmst_ifhavegds()) != 0){
			%>
