...
ArrayList<Product> productlist=getProduct(code,num);
	...
	<tr><td valign="top" height="302"><a href="<%if (pProduct.getSpgdsrcm_otherlink().trim().length()==0){%>/product/<%=product.getId()%><%}else{%><%=pProduct.getSpgdsrcm_otherlink().trim()%><%}%>" target=_blank style='text-decoration:none' title="<%=pProduct.getSpgdsrcm_gdsname()%>">
		...
		<tr><td height="13">&nbsp;</td>
		<tr><td width="300px" height="26px" align="center"  style="color:#ccc;font-size:14px;font-weight:bold;"><%=StringUtils.getCnSubstring(Tools.clearHTML(pProduct.getSpgdsrcm_gdsname()),0,34) %></td></tr>
		...
		<td><span style="font-size:24px;font-weight:bold;color:white;font-family:'微软雅黑';color:#ED7E2C">独享价:￥<%=Tools.getFormatMoney(product.getGdsmst_memberprice()) %></span></td>
		<td width="60">
		<%
		if(Tools.longValue(product.getGdsmst_validflag())!=1 || Tools.longValue(product.getGdsmst_ifhavegds()) != 0){
			%>
