...
public static int addXsYProductToCart(HttpServletRequest request,HttpServletResponse response,String code,HashMap<String,String> pmap) {
	...
	while(it.hasNext()){
		maxcount++;
		String gdsid = it.next().substring(0,8);
	...
	if(cartAdded==null){
		...
		while(it2.hasNext()){
			...
			String pid=ppid.substring(0,8);
		    String skuid=pmap.get(ppid);
			//创建套餐子项
			Product pc = (Product)Tools.getManager(Product.class).get(pid);
			Cart ccart = new Cart();
			ccart.setAmount(new Long(1));
			ccart.setProductId(pc.getId());
			ccart.setTitle(Tools.clearHTML(pc.getGdsmst_gdsname()));
			ccart.setSkuId(skuid);
...
if(Tools.isNull(skuId)){//用户没有传递sku过来。则判断物品是否需要sku，如果不需要，则直接加入购物车，如果需要，则需要弹出sku选择框
	...
	if(isHaveSku == 0){//则直接加入购物车。
		int error = addXsYProductToCart(request,response,code,goodsMap);
	...
	for(int i=0;i<size;i++){
		...
		goodsMap.put(product.getId()+i,s);
