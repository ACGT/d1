<%@ page contentType="text/html; charset=UTF-8"%><%@include file="/inc/header.jsp" %><%!private static int Exits(String mbrid)
{
	int result=0;
     ...
     clist.add(Restrictions.ge("gdsbuyonedtl_mstid",new Long(66)));
     clist.add(Restrictions.le("gdsbuyonedtl_mstid",new Long(71)));
     clist.add(Restrictions.eq("gdsbuyonedtl_mbrid",new Long(mbrid)));
     SimpleDateFormat   df2=new   SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  
     SimpleDateFormat df3=new SimpleDateFormat("yyyy-MM-dd");
     String stime="2012-10-15 00:00:00";
     String etime ="2012-10-31 23:59:59";
     try {
...
String end="2012-11-15 00:00:00";
   ...
   ArrayList<Cart> cartList = CartHelper.getCartItems(request, response);	
	...
	if(cartList!=null){
		for(Cart c123:cartList){
			if((c123.getType().longValue()==14 || c123.getType().longValue()==0)&& (gdsidlist.indexOf(c123.getProductId())>=0)){
	...
	if(hasOne){
		if(productid.equals(gid)){
			...
			out.print("{\"code\":1,message:\"活动赠品六选一，您已领取。\"}");
			return;
		}
	...
	cart.setTitle("【绽放秋日】"+productname);
