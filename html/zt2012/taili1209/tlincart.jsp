...
try{
	   	 dStartDate =fmt.parse("2012-10-1");
	 }
catch(Exception ex){
	ex.printStackTrace();
}
...
if(!"01416682".equals(gdsid) && !"01413824".equals(gdsid) && !"01205265".equals(gdsid) && !"01714989".equals(gdsid) && !"01517339".equals(gdsid)){
	out.print("{\"code\":1,message:\"参数错误！\"}");	
	return;
}
...
if(cartList!=null){
	for(Cart c123:cartList){
		if(c123.getType().longValue()==14&& (c123.getProductId().equals("01416682") ||c123.getProductId().equals("01413824") || c123.getProductId().equals("01205265") || c123.getProductId().equals("01714989") || c123.getProductId().equals("01517339"))){
			hasOne = true ;
			break;
		}
	}
...
if(ProductHelper.hasSku(product)){
	...
	ArrayList<TaiLi2012> list=getexist(tailino.replace("_9", "_1"));
	if(list!=null && list.size()>0){
		...
		cart.setTitle("【台历九月份活动】"+product.getGdsmst_gdsname());
		...
		out.print("{\"code\":1,message:\"台历号输入错误，请输入台历号_9！\"}");
	}
	return;
