   ...
   ArrayList<Cart> cartList = CartHelper.getCartItems(request, response);	
	...
	if(cartList!=null){
		for(Cart c123:cartList){
			if(c123.getType().longValue()==14 && "01205279".equals(c123.getProductId())){
	...
	if(hasOne){
		if(productid.equals(gid)){
			...
			out.print("{\"code\":1,message:\"该台历每个订单只能购买一件\"}");
			return;
		}
	...
	cart.setMoney(new Float(3));
	cart.setOldPrice(new Float(3));
	cart.setPoint(new Long(3));
	cart.setPrice(new Float(3));
	cart.setSkuId(skuId);
	...
	cart.setTitle("【+3元领取】"+productname);
