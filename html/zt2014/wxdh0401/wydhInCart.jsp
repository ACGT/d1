...
try{
	 dStartDate =fmt.parse("2014-04-09");
	 }
catch(Exception ex){
	ex.printStackTrace();
}
...
{
	out.print("{\"code\":1,message:\"对不起活动已经结束！\"}");
	return;
}
*/
...
if(strgdsdh_code!=null&&!strgdsdh_code.startsWith("pltwx1304a")){
...
String gdsid ="01516824";
//request.getParameter("gdsid");
...
if(Tools.isNull(memo)){
	...
	if(cartList!=null){
		for(Cart c123:cartList){
			...
			if(c123.getTuanCode().startsWith("pltwx1304a")){
				hastwo=true;
				break;
			}
			}
	...
	if(hastwo){
		out.print("{\"code\":1,message:\"此兑换码一个订单只能兑换一个！\"}");
		return;
	}
	if(Tuandh_mid!=48){
	...
	*/
	...
	Tools.getManager(Cart.class).create(cart);
		...
		//String pname="【Sheromo 诗若漫】百搭实用高腰显瘦弹力多色小脚铅笔裤（"+color+"）";
		out.print("{\"success\":true,\"message\":\"添加成功！\",\"cardimg\":\""+cardimg+"\",\"pname\":\"\"}");
		return;
//}
%>