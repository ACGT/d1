...
//返回到这个地址的配置在淘宝开放平台后台配置，也要用d1商城的淘宝帐户登陆才可以修改！
 ...
 if(sessionKey!=null&&sessionKey.trim().length()>0){
  	FileWriter fw = new FileWriter(new File("/var/taobao_session_key3.txt"),false);
  	fw.write(sessionKey);
  	fw.flush();
  	fw.close();
  }
  ...
  if("true".equals(request.getParameter("get"))){
	  BufferedReader br = new BufferedReader(new FileReader(new File("/var/taobao_session_key3.txt")));
