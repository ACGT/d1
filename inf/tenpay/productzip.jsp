...
response.setHeader("Content-Disposition", "attachment;filename=\"" + gdsid2 + ".zip\"");
...
String gdsstr1="03000122,03000118,03000117,03000119,03000120,03000123,03000124,03000125,03000121";
String gdsstr2="03000019,03000020,03000021";
if(gdsstr1.indexOf(gdsid)>=0){
	
	photo="<img src=\"http://images.d1.com.cn/images2013/product/tenpay9.jpg\" />"+photo;
}
if(gdsstr2.indexOf(gdsid)>=0){
	photo="<img src=\"http://images.d1.com.cn/images2013/product/tenpay3.jpg\" />"+photo;
}
...
if(true)
{
		...
		Pattern pa3 = Pattern.compile("http://images1\\.d1\\.com\\.cn/[\\-\\._/a-zA-Z0-9]+\\.jpg");
		Pattern pa4 = Pattern.compile("http://images1\\.d1\\.com\\.cn/[\\-\\._/a-zA-Z0-9]+\\.gif");
		Matcher m3 = pa3.matcher(photo);
		Matcher m4 = pa4.matcher(photo);
		...
		while(it.hasNext()){
			...
			ZipEntry ze = new ZipEntry(gdsid2+"/image/"+iname);
...
if(true){
	...
	ZipEntry ze = new ZipEntry(gdsid2+"/detail.html");
...
if(true){
	...
	*/
	...
	//创建一个ZipEntry，并设置Name和其它的一些属性
		ZipEntry ze = new ZipEntry(gdsid2+"/css/juhui_min.css");
...
if(true)
{
    ...
    detail+="<script type=\"text/javascript\" src=\"http://juhui.tenpay.com/res/js/juhui/juhui_basic/auto_set_height.js\"></script></body></html>";
    //创建一个ZipEntry，并设置Name和其它的一些属性
  	ZipEntry ze = new ZipEntry(gdsid2+"/service.html");
  	byte[] bs = detail.getBytes("UTF-8");
  	ze.setSize(bs.length);
  	ze.setTime(System.currentTimeMillis());
  	//将ZipEntry加到zos中，再写入实际的文件内容
...
if(true)
{
    ...
    //创建一个ZipEntry，并设置Name和其它的一些属性
  	ZipEntry ze = new ZipEntry(gdsid2+"/desc.html");
  	byte[] bs = detail.getBytes("UTF-8");
  	ze.setSize(bs.length);
  	ze.setTime(System.currentTimeMillis());
  	//将ZipEntry加到zos中，再写入实际的文件内容
