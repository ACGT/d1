...
String product_id="205235035812";
results.add("appid="+AppId);
results.add("product_id="+product_id);
results.add("mch_id="+MchId);
results.add("nonce_str="+nonce_str);
results.add("time_stamp="+time_stamp);
...
String gourl="weixin://wxpay/bizpayurl?";

gourl=gourl+stringA+"&sign="+sign;
...
BitMatrix bitMatrix = new MultiFormatWriter().encode(gourl,BarcodeFormat.QR_CODE, width, height, hints); 
//生成二维码 
File outputFile = new File("/opt/d1web/weixin/"+File.separator+"new2.gif"); 
RCode.writeToFile(bitMatrix, format, outputFile); 
out.print("<img src=\"/weixin/new2.gif\">");
/*reqData.append("<xml>");
...
reqData.append("<xml>");
reqData.append("<appid>"+AppId+"</appid>");
reqData.append("<mch_id>"+MchId+"</mch_id>");
reqData.append("<attach>"+attach+"</attach>");
reqData.append("<product_id>"+product_id+"<product_id>");
reqData.append("<nonce_str>"+nonce_str+"</nonce_str>");
reqData.append("<sign>"+sign+"</sign>");
reqData.append("</xml>");
