...
public static ArrayList<OrderItemMain> getOrderMainList(String cardno,Date start,Date end){
...
public static ArrayList<OrderItemRecent> getOrderRecentList(String cardno,Date start,Date end){
...
public static ArrayList<OrderItemBase> getOrderList(String cardno,Date start,Date end){
	ArrayList<OrderItemBase> list=new ArrayList<OrderItemBase>();
	ArrayList<OrderItemMain> listmain=getOrderMainList(cardno,start,end);
	if(listmain!=null){
	...
	ArrayList<OrderItemRecent> listrecent=getOrderRecentList(cardno,start,end);
	if(listrecent!=null){
    ...
    if(start.length()>0&&ends.length()<=0){
    	try{
    	    ...
    	    if(s.before(lss)||s.after(lse)){
    	    	list= getOrderList(dhcardno,lss,lse);
    	    }
    	   
    	    else
    	    {
    	    	list= getOrderList(dhcardno,format.parse(start+" 00:00:00"),lse);
    	    }
    	...
    	try{
    	    ...
    	    Date e=format.parse(ends+" 00:00:00");
			 ...
			 {
				 list= getOrderList(dhcardno,lss,format.parse(ends+" 23:59:59"));
			 }
    	}catch(Exception ex){
    	...
    	list= getOrderList(dhcardno,s,e);
    	}catch(Exception ex){
    		System.out.print(ex);
    	}
    }
    else{
    	list= getOrderList(dhcardno,null,null);
    }
