...
Award award = AwardHelper. getByProductId(gdsid);
if(award==null){
...
if("00000000".equals(award.getAward_gdsid())){//00000000直接兑换成优惠券，不用加入购物车了
	synchronized(lUser){
		...
		if(!Tools.isNull(t789.getId())){
			out.print("{\"code\":1,message:\"积分兑换优惠券成功！\"}");
			return;
		}else{
			out.print("{\"code\":1,message:\"兑换失败！\"}");
			//返回用户多扣除的积分，一般不会出现这种情况！！！！
			if(listus123!=null&&listus123.size()>0){
				UserScore us = (UserScore)listus123.get(0);
				if(us!=null){
					us.setUsrscore_realscr(new Float(us.getUsrscore_realscr().longValue()+(ppoint-total_point_for_cut)));
					Tools.getManager(UserScore.class).update(us, false);
				}
			}
			return;
