	  ...
	  $.ajax({
			...
			error: function(XmlHttpRequest){
				...
				if(json.code==1){
					...
					$.close(); 
					var s="";if((typeof c)!="undefined"){s="?c="+encodeURIComponent(c);}else{s=""+document.location;s=s.replace("http://","");s=s.substring(s.indexOf("/"));s="?c="+encodeURIComponent(s);}$.load("选择商品",400,"/market/choosesku.jsp?code="+code);
				
				}else if(json.code==2){
					 $('#imageField').attr('attr',code);
					    $.inCart(obj,{ajaxUrl:'/ajax/flow/cftdhInCard.jsp',width:400,align:'center'});
				}
			},beforeSend: function(){
			},complete: function(){
			}
...
<table id="__01" width="980"  align="center"  border="0" cellpadding="0" cellspacing="0">
	...
	List<Promotion> recommendList = PromotionHelper.getBrandListByCode("2864" , (int)21);
