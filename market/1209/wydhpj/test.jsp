...
.spgg1 .skuname1 li a:link,.spgg .skuname1 li a:visited{display:block;float:left;padding:1px;white-space:nowrap;margin:0px;height:50px;line-height:50px;overflow:hidden;border:1px solid #ccc;background:#fff;color:#ccc;text-decoration:none;cursor:pointer;}
.spgg1 .skuname1 li a:hover,.spgg .skuname1 li a:active{height:50px;line-height:50px;padding:0px;border:2px solid #ccc;text-decoration:none;}
.spgg1 .skuname1 li.select a:link,.spgg .skuname1 li.select a:visited,.spgg .skuname1 li.select a:hover,.spgg .skuname1 li.select a:active{height:50px;line-height:50px;padding:0px;border:2px solid #f50;background:#fff;color:#ccc;text-decoration:none;cursor:pointer;}
			...
			<div class="gs_right">
			    ...
			    <div class="goods_new">
					...
					<div class="gs_right_spimg">
							<img src="http://images.d1.com.cn/images2012/market/wangyi/01719683_400.jpg" width="400" height="400" />
										    		...
										    		for(int i=0;i<size;i++){
										    			...
										    			if(product.getGdsmst_stocklinkty()!=null&&(product.getGdsmst_stocklinkty().longValue()==1||product.getGdsmst_stocklinkty().longValue()==2)){
										    				if(CartItemHelper.getProductOccupyStock(product.getId(), sku.getId())<ProductHelper.getVirtualStock(product.getId(), sku.getId())){
											    				%><li<%if(size==1){ %> class="select"<%} %>><a href="javascript:void(0);" title="<%=skuname %>" attr="<%=sku.getId() %>" onclick="chooseskuname1(this)" hidefocus="true"<%if(size==1){ %> class="current"<%} %>><img  height=80 width=65 src="http://images.d1.com.cn/images2012/market/wangyi/<%=id %>_<%=sku.getId().trim()%>.jpg " />
										    		</a>
										    		<br/><br/><br/><%=skuname%></li><%
										    				}
										    				else
										    				{
										    					if(sku.getSkumst_vstock().longValue()==0){ %>
										    						<li><a href="javascript:void(0);" title="售罄"   hidefocus="true"  style="height:21px;line-height:21px;padding:0 9px;border:1px solid #dcdddd;background:#fff;color:#dcdddd;text-decoration:none;"><img  height=80 width=65 src="http://images.d1.com.cn/images2012/market/wangyi/<%=id %>_<%=sku.getId().trim()%>.jpg " />
										    		</a>
										    		<br/><br/><br/><%=skuname%></li>
										    					<%}
										    					else
  ...
  <tr>
    ...
    <div style="float:left;padding-left:10px;height:26px;_height:24px;padding-bottom:11px;"> <input type="text" style="width:180px" name="cardno" id="cardno" style="border:none;"/></div>
				...
				<div  class="zh_title"><a href="###" class="newa">网易独享</a></div>
					<div class="clear"></div>
				 <div><%
		request.setAttribute("reccode","8124");
...
function ShowAJaxdh(){
	...
	$.ajax({
		...
		url: 'wydhmincart.jsp',
		cache: false,
		data: {gdsid:'01719683',cardno:cardno,skuId:sku2,giftgds:giftgds,zpsku:zpsku},
		error: function(XmlHttpRequest){
