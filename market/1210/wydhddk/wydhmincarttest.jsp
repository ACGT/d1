...
strgdsdh_giftgds=URLDecoder.decode(strgdsdh_giftgds, "utf-8");
//System.out.println("WWWWWWWWWWWW"+strgdsdh_giftgds);
...
if(Tools.isNull(memo)){
	...
	if (!Tools.isNull(strgdsdh_giftgds)){
		strgdsdh_giftgds=strgdsdh_giftgds.substring(0, strgdsdh_giftgds.length()-1);
		String [] gdslist=strgdsdh_giftgds.split("\\|");
		for(int i=0;i<gdslist.length;i++){
		//	System.out.println("ZZZZZZZZZZZZZZZZ"+gdslist[i]);
			if(!Tools.isNull(gdslist[i])){
				Product p=ProductHelper.getById(gdslist[i]);
				if(p!=null){
					float ftuanprice2=10;
					Cart cart2 = new Cart();
					...
					cart2.setTitle("【网易兑换赠品】"+Tools.clearHTML(p.getGdsmst_gdsname()));
