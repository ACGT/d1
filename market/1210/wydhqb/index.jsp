...
<div align="center"><img src="http://images.d1.com.cn/market/1206/wydh/wydh120601.jpg" width="980" height="45"></div>	 
			...
			<div class="gs_right">
			    ...
			    <div class="goods_new">
					...
					<div class="gs_right_spimg">
							<img src="http://images.d1.com.cn/images2012/market/wangyi/01718807_400_4[1].jpg" width="400" height="400" />
										    		...
										    		for(int i=0;i<size;i++){
										    			...
										    			if(product.getGdsmst_stocklinkty()!=null&&(product.getGdsmst_stocklinkty().longValue()==1||product.getGdsmst_stocklinkty().longValue()==2)){
										    				if(CartItemHelper.getProductOccupyStock(product.getId(), sku.getId())<ProductHelper.getVirtualStock(product.getId(), sku.getId())){
											    				%><li<%if(size==1){ %> class="select"<%} %>><a href="javascript:void(0);" title="<%=skuname %>" attr="<%=sku.getId() %>" onclick="chooseskuname1(this)" hidefocus="true"<%if(size==1){ %> class="current"<%} %>><img  height=80 width=65 src="http://images.d1.com.cn/images2012/market/wangyi/<%=id %>-<%=sku.getId().trim()%>.jpg " />
										    		</a>
										    		<br/><br/><br/><%=skuname%></li><%
										    				}
										    				else
										    				{
										    					if(sku.getSkumst_vstock().longValue()==0){ %>
										    						<li><a href="javascript:void(0);" title="售罄"   hidefocus="true"  style="height:21px;line-height:21px;padding:0 9px;border:1px solid #dcdddd;background:#fff;color:#dcdddd;text-decoration:none;"><img  height=80 width=65 src="http://images.d1.com.cn/images2012/market/wangyi/<%=id %>-<%=sku.getId().trim()%>.jpg " />
										    		</a>
										    		<br/><br/><br/><%=skuname%></li>
										    					<%}
										    					else
   ...
   <tr>
    <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="15%"><input type="checkbox" name="giftgds" id="giftgds" value="01719683" onclick= "cbclick(this); "/></td>
        <td width="16%"><a href="http://www.d1.com.cn/product/01719683" target="_blank"><img src="http://images.d1.com.cn/shopadmin/d1gdsimg/2012/10/02/01719683_s_6.jpg" width="80" height="80" border="0" /></a></td>
        <td width="69%">

        <font color="#FF0000" style="font-size:24px;"><b>+39元</b></font>获得原价<font color="#FF0000" style="font-size:24px;"><b>159元</b></font>【AleeiShe 小栗舍】超大玛雅民族图案仿羊绒大披肩</td>
      </tr>
    </table></td>
  </tr>
...
<td colspan="2">
										    ...
										    if(skuList2 != null && !skuList2.isEmpty()){
												...
												<div style="clear:both;"></div>
										    		...
										    		for(int i=0;i<size;i++){
										    			...
										    			if(product.getGdsmst_stocklinkty()!=null&&(product.getGdsmst_stocklinkty().longValue()==1||product.getGdsmst_stocklinkty().longValue()==2)){
										    				if(CartItemHelper.getProductOccupyStock(product.getId(), sku.getId())<ProductHelper.getVirtualStock(product.getId(), sku.getId())){
											    				%><li<%if(size==1){ %> class="select"<%} %>><a href="javascript:void(0);" title="<%=skuname %>" attr="<%=sku.getId() %>" onclick="chooseskuname3(this)" hidefocus="true"<%if(size==1){ %> class="current"<%} %>><img  height=80 width=65 src="http://images.d1.com.cn/images2012/market/wangyi/<%=pid %>_<%=sku.getId().trim()%>.jpg " />
										    		</a>
										    		<br/><br/><br/><%=skuname%></li><%
										    				}
										    				else
										    				{
										    					if(sku.getSkumst_vstock().longValue()==0){ %>
										    						<li><a href="javascript:void(0);" title="售罄"   hidefocus="true"  style="height:21px;line-height:21px;padding:0 9px;border:1px solid #dcdddd;background:#fff;color:#dcdddd;text-decoration:none;"><img  height=80 width=65 src="http://images.d1.com.cn/images2012/market/wangyi/<%=pid %>_<%=sku.getId().trim()%>.jpg " />
										    		</a>
										    		<br/><br/><br/><%=skuname%></li>
										    					<%}
										    					else
  ...
  <tr>
  <td colspan="2"><p>&nbsp;</p><span style="color:#FC7C17;">第二步：</span>输入兑换码，放入购物车</td>
  </tr>
  <tr>
    ...
    <div style="float:left;padding-left:10px;height:26px;_height:24px;padding-bottom:11px;"> <input type="text" style="width:180px" name="cardno" id="cardno" style="border:none;"/></div>
				...
				<div  class="zh_title"><a href="###" class="newa">网易独享</a></div>
					<div class="clear"></div>
				 <div><%
		request.setAttribute("reccode","8166");
...
function ShowAJaxdh(){
	...
	$.ajax({
		...
		url: 'wydhmincart.jsp',
		cache: false,
		data: {gdsid:'01718807',cardno:cardno,skuId:sku2,giftgds:giftgds,zpsku:zpsku},
