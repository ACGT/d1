			...
			<div class="gs_right">
			    ...
			    <div class="goods_new" style="width:769px;">
					...
					<div class="gs_right_spimg">
							<img src="http://images.d1.com.cn/market/1303/wydh/03000349_400_2.jpg" width="400" height="400" />
   ...
   <tr>
    <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      %>
        ...
        <font color="#FF0000" size="+1"><b>+39元</b></font>获得市场价<font color="#FF0000" size="+1"><b><%=Tools.getFormatMoney(p.getGdsmst_saleprice()) %>元</b></font><%=Tools.clearHTML(p.getGdsmst_gdsname()) %></td>
       </tr>
    </table></td>
  </tr>
...
<td colspan="2">
           <div class="spgg1"><%
           String skuname2= p.getGdsmst_skuname1();//sku1
        	 if(!Tools.isNull(skuname2)){
									    	...
									    	//System.out.println(showsku);
										    List<Sku> skuList = SkuHelper.getSkuListViaProductIdO(zpgdsid,showsku);
										    if(skuList != null && !skuList.isEmpty()){
										    	int size = skuList.size();
										    	%><div id="skuname3" class="skuname">
										    		<p>选择<%=skuname2 %>：<font id="sizecount3"><%=size==1?skuList.get(0).getSkumst_sku1():"未选择" %></font>
										    		<% ArrayList<GdsAtt> list=GdsAttHelper.getGdsAttByGdsid(zpgdsid);
										    		   if(list!=null&&list.size()>0)
										    		   {
										    			   if(list.get(0).getGdsatt_content().length()>0)
										    			   {
										    		...
										    		for(int i=0;i<size;i++){
										    			...
										    			if(p.getGdsmst_stocklinkty()!=null&&(p.getGdsmst_stocklinkty().longValue()==1||p.getGdsmst_stocklinkty().longValue()==2)){
										    				if(CartItemHelper.getProductOccupyStock(p.getId(), sku.getId())<ProductHelper.getVirtualStock(p.getId(), sku.getId())){
											    				%><li<%if(size==1){ %> class="select"<%} %>><a href="javascript:void(0);" title="<%=skuname %>" attr="<%=sku.getId() %>" onclick="chooseskuname3(this)" hidefocus="true"<%if(size==1){ %> class="current"<%} %>><span><%=skuname%></span>	</a>									    		</a>
										    		</li><%
										    				}
										    				else
										    				{
  ...
  <tr>
    ...
    <div style="float:left;padding-left:10px;height:26px;_height:24px;padding-bottom:11px;"> <input type="text" style="width:180px" name="cardno" id="cardno" style="border:none;"/></div>
				...
				<div  class="zh_title"><a href="###" class="newa">网易独享</a></div>
					<div class="clear"></div>
				 <div><%
		request.setAttribute("reccode","8267");
...
function ShowAJaxdh(){
	...
	$.ajax({
		...
		url: 'incart.jsp',
		cache: false,
		data: {gdsid:'03000349',cardno:cardno,skuId:sku2,giftgds:giftgds,zpsku:zpsku},
		error: function(XmlHttpRequest){
