	    ...
	    <div class="goodsshow">
			...
			<div class="gs_right">
			    ...
			    <div class="goods_new">
					...
					<%=gdsname %> 
					<%if (!Tools.isNull(product.getGdsmst_title())){
						out.print("<span style=\"color:#c00;\"><br/>"+product.getGdsmst_title()+"</span>");
					} %>
					<% 
					...
					if(endflag&&flagbjcp&&rackCode.startsWith("014")){ %><a href="http://www.d1.com.cn/zhuanti/201308/fqh0815/" target="_blank">
					 <span style="color:#c00;font-size:20px;"><br/>此商品参加全场美妆满200赠100活动！活动日期：8月16日-8月28日</span></a><%} %>
					 <% if(endflag&&flagbjcp&&(rackCode.startsWith("02")||rackCode.startsWith("03")||rackCode.startsWith("015009"))){ %><a href="http://www.d1.com.cn/zhuanti/201308/fqh0815/" target="_blank">
					 <span style="color:#c00;font-size:20px;"><br/>此商品参加全场服饰满200赠100、满300赠150活动！活动日期：8月16日-8月28日</span></a><%} %><br/>
...
<li>
 ...
 rel="useZoom: 'zoom1', smallImage: 'http://images1.d1.com.cn<%=dtlimg.getGdsimgdtl_midimg() %>'">
 ...
 <%} %>
					...
					<!-- 商品展示结束-->
				     ...
				     <div class="gs_right_spContent">
                         ...
                         <tr><td>
                        <img src="http://images.d1.com.cn/images2013/product/shop.png"></img> <%
                        ...
                        %>
                         ...
                         <%if (flagbj200){
                        	 %>
                        	 <tr>
									        <td colspan="3" height="30">市场价：￥<%=Tools.getFormatMoney(saleprice) %></span></td>
								          </tr>
                        	 <tr>
								        <td colspan="3" height="30">会员价：<span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%= Tools.getFormatMoney(memberprice) %></span></td>
							     </tr>
                        	 <%
                         }else{
                              if(lUser!=null&&UserHelper.isPtVip(lUser)&&pbjdx!=null&&pbjdx.getSpgdsrcm_tjprice()!=null&&pbjdx.getSpgdsrcm_tjprice().floatValue()!=0)
                              {%>
                            	 <tr>
								        <td colspan="3" height="30">会员价：￥<%= Tools.getFormatMoney(memberprice) %></span></td>
							     </tr>
							    <tr>
							      <td colspan="3" >促销价：<span style="background-color: whiteSmoke;border: 1px solid #F0EDEC;border-bottom: 1px dotted #999;color: #999;height: 16px;line-height: 16px;margin: 6px 8px 0 0;padding: 0 3px;">白金独享价</span><span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=(int)pbjdx.getSpgdsrcm_tjprice().floatValue() %></span></td>
						        </tr>
						    	<%
						     }						    
                              else
                              {
                            	 ...
                            	 if(ismiaoshao||isdx||(tuanflag==1&&Tools.floatCompare(tuanprice , oldmemberprice) != 0)||(lUser!=null&&UserHelper.isPtVip(lUser))||(lUser!=null&&UserHelper.isVip(lUser))){
                            		 if((ismiaoshao&&(lUser==null||(lUser!=null&&!UserHelper.isPtVip(lUser)&&!UserHelper.isVip(lUser))))||(tuanflag==1&&Tools.floatCompare(tuanprice , oldmemberprice) != 0)){
							    	 if(isdx&&!ismiaoshao)
							    	 {%>
							    		 <tr>
								        <td colspan="3" height="30">会员价：￥<%=Tools.getFormatMoney(memberprice) %></span></td>
							    	 ...
							    	 {
							    		 if(tuanflag==1&&Tools.floatCompare(tuanprice , oldmemberprice) != 0&&lUser!=null&&UserHelper.isPtVip(lUser))
							    		 {%>
							    			 <tr>
								        <td colspan="3" height="30">会员价：￥<%=Tools.getFormatMoney(memberprice) %></span></td>
							          </tr>
							    	 ...
							    	 %>
							    		 <tr>
								        <td colspan="3" height="30">会员价：￥<%=Tools.getFormatMoney(oldmemberprice) %></span></td>
							          </tr>
							    	 <%}
							    	 }
							    	 }  
							    	 else if(lUser!=null&&UserHelper.isVip(lUser)&&!UserHelper.isPtVip(lUser)&&(rackCode.startsWith("014")||rackCode.startsWith("015002")))
							    	 {%>
							    		 <tr>
									        <td colspan="3" height="30">市场价：￥<%=Tools.getFormatMoney(saleprice) %></span></td>
								          </tr>
								     ...
								     {
								    	 endprice=String.valueOf(memberprice);
								     %>
								     <tr>
									      <td colspan="3" height="30">会员价：<span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=Tools.getFormatMoney(memberprice) %></span></td>
								     </tr>
								     <%}                            	      
                                      if(lUser!=null&&UserHelper.isPtVip(lUser)&&!isdx)
		                           	  {
		                           		endprice=String.valueOf(bjprice);
		                           	  %>
		                           		   <tr>
									      <td colspan="3" >促销价：<span style="background-color: whiteSmoke;border: 1px solid #F0EDEC;border-bottom: 1px dotted #999;color: #999;height: 16px;line-height: 16px;margin: 6px 8px 0 0;padding: 0 3px;">白金价</span><span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=bjprice %></span></td>
								         </tr>
		                           	  <%}		                           	 
		                           	  else
		                           	  {
								        if(ismiaoshao||isdx||(tuanflag==1&&Tools.floatCompare(tuanprice , oldmemberprice) != 0)||(lUser!=null&&UserHelper.isVip(lUser)))
								        {
								    	//获取活动特价
										 if(isdx){
										 %><tr><td colspan="3"><%
											 if(!Tools.isNull(dxtitle)){
												 ...
												 endprice=String.valueOf(dxprice);
												 hyprice = dxprice;
												 //System.out.println("dxxxxxxxxxxxx1");
												 %>促销价：<span style="background-color: whiteSmoke;border: 1px solid #F0EDEC;border-bottom: 1px dotted #999;color: #999;height: 16px;line-height: 16px;margin: 6px 8px 0 0;padding: 0 3px;">独享价</span><span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=Tools.getFormatMoney(dxprice) %></span><%
														
											 }
											 else
											 {
												if(tuanflag==1&&Tools.floatCompare(tuanprice , oldmemberprice) != 0)
												 {
											    //获取团购价
												     endprice=String.valueOf(tuanprice);
													 hyprice = tuanprice;
													 %><tr><td>促销价：<span style="background-color: whiteSmoke;border: 1px solid #F0EDEC;border-bottom: 1px dotted #999;color: #999;height: 16px;line-height: 16px;margin: 6px 8px 0 0;padding: 0 3px;">团购价</span><span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=Tools.getFormatMoney(tuanprice) %></span><%
												%></td></tr>
													 <%
												 }
												 else if(ismiaoshao)
												 ...
												 %>
												 <tr>
											      <td colspan="3" >促销价：<span style="background-color: whiteSmoke;border: 1px solid #F0EDEC;border-bottom: 1px dotted #999;color: #999;height: 16px;line-height: 16px;margin: 6px 8px 0 0;padding: 0 3px;">秒杀价</span><span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=Tools.getFormatMoney(memberprice) %></span></td>
										         </tr>
												<% }
											 }
										 %></td></tr><%
							    		 ...
							    		 {
											 endprice=String.valueOf(vipprice);
							    		 %>
							    			 <tr>
								             <td colspan="3" >促销价：<span style="background-color: whiteSmoke;border: 1px solid #F0EDEC;border-bottom: 1px dotted #999;color: #999;height: 16px;line-height: 16px;margin: 6px 8px 0 0;padding: 0 3px;">VIP价</span><span style="color: #C00000;font-size: 22px;font-family:'Arial';">￥<%=vipprice %></span></td>
							                 </tr> 
							    		 <%}
										 else if(tuanflag==1&&Tools.floatCompare(tuanprice , oldmemberprice) != 0)
										 {
									    //获取团购价
										     endprice=String.valueOf(tuanprice);
											 hyprice = tuanprice;
								    ...
								    if(isdx&&(lUser==null||(lUser!=null&&!UserHelper.isPtVip(lUser)&&!UserHelper.isVip(lUser))))
								    {
								    	
								    }
								    else
								    {
									     if(lUser==null||(!UserHelper.isPtVip(lUser)&&!UserHelper.isVip(lUser))){ %>
										 <tr>
										 <td colspan="3" height="25">VIP价 ：￥<%= vipprice %></td>
							             </tr>
							             <%} %>
							             <%if((lUser==null||!UserHelper.isPtVip(lUser))){
							            	 if((lUser!=null&&UserHelper.isVip(lUser)&&isdx))
							            	 {} 
			                                   else
			                                   {%>
			                                	 <tr>
											    <td colspan="3" height="25">白金价：￥<%= bjprice %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id="hyyhsmLink" href="javascript:void(0)">如何成为白金会员</a>
								      <div class="hyyhsm1" id="hyyhsm" style="display:none">
									    <span>成为VIP会员：</span><br/>
									 &nbsp;&nbsp;只需年度累计购物金额超过<font style="color:#f00">500</font>元即可！<br/>
									 <hr/>
									<span> 成为白金会员：</span><br/>
									 &nbsp;&nbsp;只需年度累计购物金额超过<font style="color:#f00">2000</font>元即可;
									 <!-- 或提前充值<font style="color:#f00">1000</font>元即可！<font style="color:#f00">（VIP会员充值500元）&nbsp;&nbsp;点击充值>> -->
									 </font><br/>
							...
							if(ifhavegds == 0){
									...
									if(ProductStockHelper.canBuy(product)){
										 ...
										 { 
									            ...
									            if(ifhaveDate1 != null&&ifhaveDate1.after(new Date())) {
									            	if(skuList != null && !skuList.isEmpty()){%>
									            	 <a href="javascript:void(0)" onclick="ShowAJax1('<%=id %>')"><img id="gwc0717" src="http://images.d1.com.cn/images2012/New/frgwc.gif" /></a>
							 ...
							 &nbsp;&nbsp;<a href="###" onclick="addFavorite('<%=id %>');"><img src="http://images.d1.com.cn/images2012/New/jrsc.gif" /></a><%
									...
									if(id.equals("02001470")||id.equals("03000384")
										...
										%>
										<img src="http://images.d1.com.cn/images2013/product/shouqin.jpg" align="absmiddle" />
										<%
									}else{
									%><img src="http://images.d1.com.cn/images2012/New/product/yxj.gif" align="absmiddle" /><%
									}
