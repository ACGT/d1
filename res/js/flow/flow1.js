...
$(document).ready(function(){
	...
	$('#checkout').live('click',function(){
		$.post("/ajax/flow/checkAllCart1.jsp",{"m":new Date().getTime()},function(json){
			if(json.error > 0){
				//购物车为空
				 if(json.error == 2){
					 CartEmptyResponse(json);
...
function fmmap(c,lastmoney,type){$.close();  var s="";if((typeof c)!="undefined"){s="?c="+encodeURIComponent(c);}else{s=""+document.location;s=s.replace("http://","");s=s.substring(s.indexOf("/"));s="?c="+encodeURIComponent(s);}$.load("结账提醒",680,"/ajax/dialog/fmcap1.jsp"+s+"&lastmoney="+lastmoney+"&type="+type);}
...
function getCartInfo(){
	$.post("/ajax/flow/getCartInfott.jsp",{"m":new Date().getTime()},function(json){
		if(json.error == 0){
