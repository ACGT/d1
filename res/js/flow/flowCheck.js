...
function CheckREmail(strREmail){
   ...
   //     return false;
  //  }
    if (!isEmail(strREmail) && strREmail.length>0){
...
function CheckRZipcode(strRZipcode){
   ...
   //     return false;
  //  }
    if (!isPostalCode(strRZipcode) && strRZipcode.length>0){
    ...
    $.ajax({
        ...
        error: function(XmlHttpRequest, textStatus, errorThrown){
        	...
        	if(json.success){
                ...
                $('#spand1ActValue').html(json.D1ActValue);
    ...
    $.ajax({
        ...
        success: function(json){
            if(json.success){
                ...
                top.location.href='/flowDone.jsp';
            }else{
                alert(json.message);
                $('#divBtnOrder').show();
            	$('#Submit33').attr("disabled",false);
