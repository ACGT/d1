...
function xpsdscroll(obj)
{
	var sWidth = $("#xpsdfocus").width();
	var len =0; 
	if($("#xpsdfocus ul li").length%4>0)
    {
        len=parseInt($("#xpsdfocus ul li").length/4+1);
    }
	else
	{
		len=parseInt($("#xpsdfocus ul li").length/4);
	}
	...
	//$("#xpsdfocus ul").css("width",sWidth * (len));
	...
	function showPics(index) { //普通切换
		...
		$("#xpsdfocus ul").stop(true,false).animate({"left":nowLeft},300); //通过animate()调整ul元素滚动到计算出的position
		
	}
}
$(function(){
	$('.hottd').each(function(i){
		
		$(this).mouseover(function(){
			//$('.hotcontent').each(function(j){
				//if(i==j)
					//{
					  //$(this).css("border","solid 1px #9c0001");
					//}
			//});
			$(this).find(".hottile a").css("color","#fff");
			$(this).css("background","#9c0001");
		...
		$(this).mouseout(function(){
			//$('.hotcontent').each(function(j){
				//if(i==j)
					...
					//}
			//});
			$(this).find(".hottile a").css("color","#2e2e2e");
			$(this).css("background","#e1e1e1")
			$(this).css("border-bottom","solid 1px #fff");
			
		});
	});
$('.hotb').each(function(i){
		
		$(this).mouseover(function(){
			$('.hottd').each(function(j){
				if(i==j)
					{
	...
	//$('.hotcontent').each(function(i){
		
		//$(this).mouseover(function(){
			//$('.hottile').each(function(j){
				//if(i==j)
					//{
					//$(this).find("a").css("color","#fff");
				//	$(this).parent().css("background","#9c0001");
		...
		//$(this).mouseout(function(){
			//$('.hottile').each(function(j){
				//if(i==j)
					...
					//$(this).parent().css("background","#e1e1e1");
	...
	$('.hotclass td span').each(function(i){		
		$(this).mouseover(function(){
			$('.hotclass td span a').each(function(j){
				if(i==j)
					{
					  $(this).css("color","#eecccc");
					}
			});
			
			  $(this).css("background","#a40000");
...
function view_time2(){
	...
	var endDate= new Date("2012/07/20 11:59:59");
