...
function ShowCenter(imglist, alist) {
...
//    var urls = ['#', '#', '#', '#', '#'];
    var widths = 328; var heights = 200; var times = 2000;
    for (i = 0; i < images.length; i++) { eval('img' + i + '=new Image ();img' + i + '.src="' + images[i] + '";'); }
    document.write('<div class="container" id="CenterImg"  style="width:' + widths + 'px;height:' + heights + 'px; margin:0px auto;">');
    ...
    var nums = [];
    for (var i = 0, n = st._count - 1; i <= n; ) { (nums[i] = $w("idNum").appendChild(document.createElement("li"))).innerHTML = ++i; }
...
SlideTrans.prototype = {
    ...
    Run: function(index) {
        ...
        this._target = -Math.abs(330) * (this.Index = index);
