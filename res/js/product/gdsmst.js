...
switch_tags.prototype.bind_switch = function() {
    ...
    //延时切换		
    function sw(m) {
    	var obj=$('#goodsinfotab');
 		if(obj.length>0)
 		{
 			obj.removeClass('newbanner1120');	
 		}
...
function choosesku20120717(obj){
  ...
  //换图片
    ...
    if(flag==1)
    {
    	...
    	{
    	$('#gwc0717').attr("src","http://images.d1.com.cn/images2014/product/frgwc.png");	
    	}
}
...
//获取评论
function get_comment2(id,pg){
	$('#commentContent').html("<div align='center'><img src='http://images.d1.com.cn/images2012/New/Loading.gif' /></div>");
	$.post("/ajax/product/commentList3.jsp",{"id":id,"pg":pg,"m":new Date().getTime()},function(data){
		$('#commentContent').html(data);
	});
}
//新的搭配价格变化
function selectInits1(mPrice,value,flag,id) {
    var zk=0.95;
    var s=$('#span'+id+flag);
    var memberP = $("#memberP"+flag);
    var cheap = $("#cheap" + flag);
...
//新的最佳组合JS
function selectInit1(mPrice, pPrice, value,flag,id) {
	var s=$('#span'+id+flag);
    var memberP = $("#memberP"+flag);
    var cheap = $("#cheap" + flag);
    var pktP = $("#pktP" + flag);
...
//新的产品页推荐搭配JS
function choosedptj1(mPrice,value,flag,id) {
	var zk=0.95;
	var s=$('#spandp'+id+flag);
    var memberP = $("#memberPstj"+flag);
    var cheap = $("#cheapstj" + flag);
    var pktP = $("#pktPstj" + flag);
    var mm=$("#memberPstj"+flag);
...
//新的产品页搭配JS
function choosedp1(mPrice,value,flag,id) {
var zk=0.95;
var s=$('#spandps'+id+flag);
var memberP = $("#memberPs"+flag);
var cheap = $("#cheaps" + flag);
var pktP = $("#pktPs" + flag);
var mm=$("#memberPs"+flag);
