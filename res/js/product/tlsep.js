...
(function($) {
	...
	$.inCart1.defaults = {
		method : 'post',
		width : 400, //total width of inCart1.  auto-adjusts based on showArrow value
		title : '', //window title
		contentClass : 'box_content', //content css class
		succButton : true, // is show success button
		ajaxUrl:'/html/zt2012/taili1209/tlincart.jsp',
	...
	$.inCart1.moreSku = function(obj,o,json){
		try{
			...
			//init
			var $inCart1 = $('#inCart1Div'),$sel_wrap = $('#tc_sel_attr_wrap');
			$.each(json.list,function(i,n){
				...
				goods.html('<p class="tc_stock_item"><a href="'+n.url+'" title="'+n.title+'" target="_blank"><img src="'+n.pic+'" /></a><span class="title" title="'+n.title+'">'+n.title+'</span></p>\
					<div class="tc_stock_buyinfo">\
						...
						<dl class="choice">\
							<dt>提示：</dt>\
							<dd id="bind_choice'+i+'">请您选择 “'+n.skuname+'”</dd>\
						</dl></div>');
		...
		try{
			...
			<div class="cart_property_wrapper">\
				...
				<div class="block_options"'+(o.goodsType==1?' style="display:none;"':'')+'>\
					<input type="hidden" tag="buyNum" value="1" name=""/> \
				</div>\
			...
			$.inCart1.init(obj,o,template);
			//init
			var $inCart1 = $('#inCart1Div');
			var select_area = $inCart1.find('div[tag=select_area]'),warn_info = $inCart1.find('p[tag=warning]'),options_sku = $('#options_sku'),buy_num = $inCart1.find('input[type=text][tag=buyNum]'),add_confirm = $inCart1.find('.add_confirm');
			var options_html = '';
			...
			add_confirm.click(function(){
				...
				if(lis.length==0){
					select_area.hide();
					warn_info.show().find('span[tag=info]').html('请选择“'+$inCart1.find('div[tag=item_name]').attr('v')+'”');
				}else{
	...
	$.inCart1.refreshState = function(){
		...
		var lis = $('#options_sku > li[selected=1]');
		if(lis.length==0){
			$inCart1.find('div[tag=select_area] > .txt_display').html('请选择<span class="color_orange">“'+$inCart1.find('div[tag=item_name]').attr('v')+'”</span>');
		}else{
			$inCart1.find('div[tag=select_area] > .txt_display').html('已选择<span class="color_orange">“'+$inCart1.find('div[tag=item_name]').attr('v')+'”</span>');
		}
	};
	$.inCart1.success = function(obj,o,json){
		var template = '<div class="box_hint_normal">\
			...
			<div class="hint_content">\
				...
				<div class="hint_op"'+(o['succButton']?'':' style="display:none;"')+'><a href="/flow.jsp" target="_blank" id="shopcar_b_watch" title="立即结算"><img src="/res/images/product/chakan.gif" /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="###" id="shopcar_b_goon" title="继续购物"><img src="/res/images/product/jixu.gif" /></a></div>\
			</div>\
		</div>';
		o['title']='提示';
		$.inCart1.init(obj,o,template);
	...
	$.inCart1.loading = function(obj,o){
		var template = '<div class="loading"><img src="/res/images/result/icon_loading_large.gif" /><strong>请稍候...</strong></div>';
		o['title']='';
		$.inCart1.init(obj,o,template);
		$.inCart1.show(o,obj);
	};
	...
	$.inCart1.init = function(obj, o , content) {
		var $inCart1 = $('#inCart1Div');
		if($inCart1.length == 0){
			$inCart1 = $('<div id="inCart1Div" class="module_box_normal"></div>');
			$inCart1.html('<div class="box_title"><h4 id="inCart1Div_title"></h4><a href="###" class="bt_close" id="inCart1Div_closer"></a></div><div id="inCart1Content"></div>');
			$(document.body).append($inCart1);
			$('#inCart1Div_closer').click(function(){
				$.inCart1.close();
			});
		}
		$('#inCart1Div_title').html(o.title);
		$('#inCart1Content').addClass(o.contentClass).html(content);
	};
