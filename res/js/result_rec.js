...
function getFloatdp(gdsid,count)
{
	...
	if(obj!=null)
	{
		if(obj1.offset().left<200){
			...
			$.post("/html/resulttest.jsp",{"gdsid":gdsid,"count":count},function(data){
				$(obj).html(data);
				$(obj).removeClass("floatdpr");
				$(obj).addClass("floatdp2");
				
				$("#price"+count).removeClass("dpprice1");
				$("#price"+count).addClass("dpprice2");
				selectdp(1,count);
			});
			...
			$.post("/html/resulth.jsp",{"gdsid":gdsid,"count":count},function(data){
				...
				$("#price"+count).removeClass("dpprice2");
				$("#price"+count).addClass("dpprice1");
				selectdp(1,count);
			});
...
function selectdp(flags,flag) {
	...
	if(flag%4==1){
		var len=$(".floatdp2").find("input[type='checkbox'][name='chk_"+flag+"_"+flags+"']:checked").length;
		$("#count_"+flag).html(len);
		var money=0;
		var tmoney=0;
		var zk=0.95;
		$(".floatdp2").find("input[type='checkbox'][name='chk_"+flag+"_"+flags+"']:checked").each(function(){
...
function AddInCart(obj){
	...
	if(flag%4==1){
	    var list=$(".floatdp2").find("input[type='checkbox'][name='chk_"+flag+"_"+flags+"']:checked");
	}
	else{
