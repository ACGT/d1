...
function ShowCenter(divid,imgid,numid,imglist, alist) {
...
//    var urls = ['#', '#', '#', '#', '#'];
    var widths = 220; var heights = 100; var times = 1000;
    for (i = 0; i < images.length; i++) { eval('img' + i + '=new Image ();img' + i + '.src="' + images[i] + '";'); }
    document.write('<div  id="'+divid+'"  style="width:' + widths + 'px;height:' + heights + 'px;">');
    document.write('<table id="'+imgid+'" class="imgtable" border="0" cellpadding="0" cellspacing="0"><tr>');
    ...
    document.write('</tr></table><ul class="num" id="'+numid+'"></ul></div>');
    var st = new SlideTrans(divid, imgid, i, times, { Vertical: false });
    ...
    var nums = [];
    for (var i = 0, n = st._count - 1; i <= n; ) { (nums[i] = $w(numid).appendChild(document.createElement("li"))).innerHTML = ++i; }
