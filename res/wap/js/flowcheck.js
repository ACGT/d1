		...
		success: function(json){
				...
				if(json.status=="1"){
				  if(t=="1"){
					$(".m_addlist ul").html();
				  showitem(json);
				  $(".m_addlist ul").append('<div class="addrnone"><a href=\"/wap/user/addro.html\">添加收货人地址</a> </div>');
				  }else{
				  chooseitem(json);
				  }
				  $("#addr").click(function(){
					...
					window.location.href='/wap/login.html';
					  ...
					  $(".m_addlist ul").html('<div class="addrnone"><a href=\"/wap/user/addro.html\">添加收货人地址</a> </div>');
					  $(".m_addlist").show();
	  ...
	  $.ajax({
		...
		success: function(json){
		 if(json.status=="-1"){
			 ...
			 var payhtml='<span><input class="pay" type="radio" checked id="payid1" name="payid" onclick="loadprice();" value="20">在线支付</span>';
			 if(json.ifCanHF){
				 payhtml+='<span><input class="pay" type="radio" id="payid2" name="payid" onclick="loadprice();" value="0">货到付款</span>';
			 }
			 $(".m_pay .item").append(payhtml);
...
function showitem(json){
	...
	if(addresss.length>0){
	   for(var i=0;i<addresss.length;i++){
		   ...
		   addrhtml=addrhtml+'<div class="edit"><a href="/wap/user/addro.html?mcstid='+addresss[i].rid+'">修改</a></div>';
		   addrhtml=addrhtml+'</div>';
...
function sendupdate(){
    ...
    $.ajax({
        ...
        success: function(json){
            if(json.success){
            	$('#sndOK').hide();
                top.location.href='/wap/flowdone.html';
            }else{
                alert(json.message);
                $('#msgerr').show();
