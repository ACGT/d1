    ...
    <!--  <div style="width:980px; margin:0px auto;text-align:center;color:#f00; font-size:15px;">
     网购温馨提示：电话诈骗猖獗，接到任何购物网站通知中奖或低价促销电话，均为诈骗信息，D1优尚也从不通过电话进行促销，请提高警惕
             ...
             if(UserScoreHelper.getRealScore(lUser.getId())>200){
            	 %>
            	  <div style="width:980px; margin:0px auto; text-align:center; font-size:22px; line-height:28px; margin-bottom:10px;"><a href="http://www.d1.com.cn/temai/" target="_blank">2013年7月1日之前的积分即将清零，兑换优惠券和好礼》》</a> </div><!-- 发表评论、晒单均可获得积分！ -->
     ...
     <%@include file="/user/left.jsp" %>
      <!--右侧-->
   <div class="mbr_right">
		  <div class="mbr_right_first" style="height:190px;">
					 <div class="mbr_info">
					 ...
					 <div>
					  <% if(lUser.getMbrmst_specialtype()==0){ %>
							...
							if(!Tools.isNull(lUser.getMbrmst_name()) && !Tools.isNull(lUser.getMbrmst_postcode()) && lUser.getMbrmst_birthday()!=null && !Tools.isNull(lUser.getMbrmst_usephone())){
								isdetail=true;
							} %>
							</div>
							...
							<div style="padding-left:8px;">
								...
								<%}else{ %>
								<a  href="profile.jsp"><s></s>手机已验证</a><br/>
								<%} if(!isemailflag){%>
								<a class="mailn" href="/newlogin/valiemail.jsp" ><s></s>邮箱未验证</a><br/>
								<%}else{ %>
								<a class="mail" href="profile.jsp" ><s></s>邮箱已验证</a><br/>
								<%} if(isdetail){%>
								<a class="spsd" href="profile.jsp" ><s></s>个人信息已完善</a><br/>
								<%}else{ %>
								<a class="spsdn" href="profile.jsp" ><s></s>个人信息未完善</a><br/>
								<%} %>
								</div>
							</div>
							...
							<% if (Tools.isNull(chePingAn)){%> 
							 您的积分：<span><b><%= (int)(UsrPointHelper.getRealScore(lUser.getId())+0.5) %></b><font color="#333">分，</font><a href="/help/helpnew.jsp?code=0104" target="_blank">积分规则</a>&nbsp;&nbsp;
