					...
					<tr><td> <img src="<%=smallimg%>" width="50" height="50" style=" float:left; vertical-align:bottom" /></td></tr>
					 ...
					 <%} %>
								       ...
								       <td>
								          ...
								          %><%if(ob.getOdrmst_orderstatus().longValue()>=3&&oi.getOdrdtl_shipstatus().longValue()>=2){
								        	  ...
								        	  if(odrshopth==null){
								        	  %>
								        	  </br><A href="/user/odrth.jsp?odrid=<%=orderid%>&subodrid=<%=subodrid %>" target="_blank">退换货申请</A>
								        	  <%}else{
								        		  ...
								        		  if(thtype == 1){//退货
								        			  ...
								        			  if(lstatus == 0){
								        				  %>
								        				  </br><A href="/user/odrstatusdetail.jsp?odrid=<%=orderid%>&subodrid=<%=subodrid %>&thtype=1&lstatus=0" target="_blank">退货进行中</A>
								        				  <%
								        			  }else if(lstatus == 1){
								        				  %>
								        				  </br><A href="/user/odrstatusdetail.jsp?odrid=<%=orderid%>&subodrid=<%=subodrid %>&thtype=1&lstatus=1" target="_blank">已退货(待退款)</A>
								        				  <%
								        			  }else{
								        				  %>
								        				  </br><A href="/user/odrstatusdetail.jsp?odrid=<%=orderid%>&subodrid=<%=subodrid %>&thtype=1&lstatus=2" target="_blank">退货(已退款)</A>
								        				  <%
								        			  }
								        		  }else{//换货
								        			  //0待受理 1已受理 2已换货
								        			  if(lstatus == 0){
								        				  %>
								        				  </br><A href="/user/odrstatusdetail.jsp?odrid=<%=orderid%>&subodrid=<%=subodrid %>&thtype=2&lstatus=0" target="_blank">换货进行中</A>
								        				  <%
								        			  }else{
								        				  %>
								        				  </br><A href="/user/odrstatusdetail.jsp?odrid=<%=orderid%>&subodrid=<%=subodrid %>&thtype=2&lstatus=1" target="_blank">换货(已换货)</A>
								        				  <%
								        			  }
								        		  }
							   ...
							   if (Tools.isNull(chePingAn)){
								   if(points > 0){
									   if(comment != null || Tools.dateValue(ob.getOdrmst_shipdate()) < System.currentTimeMillis()-Tools.MONTH_MILLIS){
										   %>您本次购物已获得积分：<font color="#e2403e"><%=ProductGroupHelper.getRoundPrice(new Float(points)) %></font>分<%
									   }else{
										   %>您本次购物将获得积分：<font color="#e2403e"><%=ProductGroupHelper.getRoundPrice(new Float(points)) %></font>分<%
									   }
								   }
							   }
