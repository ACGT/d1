...
<body>
	...
	<div class="all">
		<span style="background: #f00; color: #fff;">季化妆品节第二波，大牌抢先团</span> <br />
		<div class="box">
			<% String result=GetZtList("7523",pages,pagesize);
			   out.print(result);
			   if(result.length()>0)
			   {%>
			<a href="<%=request.getRequestURI() %>?page=<%=pages-1 %>">上一页</a> <a
				href="<%=request.getRequestURI() %>?page=<%=pages+1 %>">下一页</a>
			<form action="<%=request.getRequestURI() %>?act=r" method="post">
				<input type="text" id="pages" name="pages" style="width: 50px;" />/<%= count %>页
