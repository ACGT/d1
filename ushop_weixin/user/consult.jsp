...
<body>
	...
	<div style="margin-bottom: 15px;">
		<div style="background: #FFDEAD; padding: 3px; width: 100%;">
			<a href="/mindex.jsp">首页</a>><a href="index.jsp">我的优尚</a>>我的商品咨询 <br />
		</div>
		<form action="consult.jsp">
			<%  if((asklist!=null&&asklist.size()>0)||(askcachelist!=null&&askcachelist.size()>0))
    {%>
			&nbsp;共
			<font color='#f00'><%= totalcount %></font>条咨询，
			<font color='#f00'><%= replaycount %></font>条已回复
			<br />
			<table>
				<%  
    	 if(asklist3!=null){
			   for(GoodsAsk goodsask:asklist3){
				...
				<tr>
					<td><b>&nbsp;<a
							href="/wap/goods.jsp?productid=<%= goodsask.getGdsask_gdsid() %>"><%= goodsask.getGdsask_gdsname() %></a></b></td>
				</tr>
				<tr>
					<td>&nbsp;<font color="#f00"><%=Tools.clearHTML( goodsask.getGdsask_content())%></font></td>
				</tr>
				<tr>
					<td>&nbsp;D1回复：<%= goodsask.getGdsask_replyContent() %></td>
				</tr>
				<%
	    		    }
    	 }
    	  if(asklist2!=null){
			   for(GoodsAskCache goodsaskcache:asklist2){
				...
				<tr>
					<td><b>&nbsp;<a
							href="/wap/goods.jsp?productid=<%= goodsaskcache.getGdsask_gdsid() %>"><%= goodsaskcache.getGdsask_gdsname() %></a></b></td>
				</tr>
				<tr>
					<td>&nbsp;<font color="#f00"><%=Tools.clearHTML( goodsaskcache.getGdsask_content())%></font></td>
				</tr>
				<tr>
					<td>&nbsp;未回复</td>
				</tr>
        ...
        %>
			...
			<div class="Pager">
              ...
              if (currentPageIndex>1 ){%>
				<a href="consult.jsp?pageno=<%=currentPageIndex-1%>"><font
          ...
          if (currentPageIndex<pagecount ){%>
				...
				<%}%>
				<a href="consult.jsp?pageno=<%=pagecount%>">尾页</a> &nbsp;&nbsp;<input
					type="text" name="pageno" style="width: 50px;" id="page"
					value="<%=currentPageIndex%>" />/<%=pagecount %>
				&nbsp;&nbsp;<input type="submit" value="跳转" style="padding: 3px;" />
			</div>
			<%}
    else
    {%>
			<font color='#f00'>您还没有进行商品咨询！</font>
			<br />
