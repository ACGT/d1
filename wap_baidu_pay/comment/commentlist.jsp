...
%>
	<div style="margin-bottom: 15px;">
		...
		<div style="background: #f00; color: #fff;">
			...
			<a href="/wap/goods.jsp?productid=<%= id %>" style="color: #fff;"><b>详情</b></a>&nbsp;|&nbsp;<a
		...
		<table>
			...
			<tr>
				<td>&nbsp;品牌：[<%= product.getGdsmst_brandname() %>]
				</td>
			</tr>
			<tr>
   ...
   List<Comment> commentlist=CommentHelper.getCommentList(id, (currentPageIndex-1)*pagesize, pagesize);
	 ...
	 for(Comment comment:commentlist){
		 ...
		 %>
			<tr style="background: #FFDEAD">
				<td colspan="2">&nbsp;来自：<%=hfusername %> &nbsp;&nbsp;&nbsp;<%=Tools.stockFormatDate(comment.getGdscom_createdate()) %></td>
			</tr>
			<tr>
				<td width="100px">&nbsp;评分：<%=comment.getGdscom_level() %>星 |
				</td>
		...
		<form action="commentlist.jsp">
			...
			<%
    		   if(currentPageIndex>1){
    			   %>
			<a
				href="<%=request.getRequestURI()%>?productid=<%=id %>&pageno=<%= currentPageIndex==1?1:currentPageIndex-1%>">上一页</a>
			<% }
			...
			&nbsp;
			<a
				href="<%=request.getRequestURI()%>?productid=<%=id %>&pageno=<%= currentPageIndex==pagecount?currentPageIndex:currentPageIndex+1%>">下一页</a>
			<%  
    		  }
    		  %>
	...
	%>
		...
		<br />&nbsp;<a href="/wap/f_succ.jsp?id=<%=id%>">收藏</a>&nbsp;&nbsp;<a
			href="/wap/goods.jsp?productid=<%=id %>">返回商品详情>></a>
		<div style="background: #f00; color: #fff;">
			&nbsp;<a href="/wap/goods.jsp?productid=<%= id %>"
				style="color: #fff;"><b>简介</b></a>&nbsp;|&nbsp;<a
				href="/wap/imglist.jsp?productid=<%= id %>" style="color: #fff;"><b>图片</b></a>&nbsp;|&nbsp;
		...
		<div style="background: #a52a2a; padding: 2px; margin-top: 2px;">
			...
			<form id="search_1" method="post"
				action="commentlist.jsp?act=search1">
