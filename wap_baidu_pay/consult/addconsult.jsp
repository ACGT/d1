...
<body>
	...
	<div style="margin-bottom: 15px;">
		<div style="background: #FFDEAD; padding: 3px; width: 100%;">
			<a href="/mindex.jsp">首页</a>>我要咨询 <br />
		</div>
		<%
    String msg="";
    String content="";
    ...
    %>
       ...
       {%>
		<form action="addconsult.jsp" method="post">
			...
			&nbsp;商品名称：
			<a href="/wap/goods.jsp?productid=<%= goodsid%>"><%= Tools.clearHTML(p.getGdsmst_gdsname()) %></a>
			<br /> &nbsp;咨询类型：
			<input name="asktype" type="radio" value="1"
				<%if("1".equals(type)){%> checked="checked" <%} %> />商品咨询
			</input>
			<input name="asktype" type="radio" value="2"
				<%if("2".equals(type)){%> checked="checked" <%} %>>库存及配送
			</input>
			<input name="asktype" type="radio" value="3"
				<%if("3".equals(type)){%> checked="checked" <%} %>>支付问题
			</input>
			<input name="asktype" type="radio" value="4"
				<%if("4".equals(type)){%> checked="checked" <%} %>>发票及保修
			</input>
			<input name="asktype" type="radio" value="5"
				<%if("5".equals(type)){%> checked="checked" <%} %>>促销及赠品
			</input>
			<br /> &nbsp;请输入提问内容(100字以内)：
			<br /> &nbsp;
		...
		<%}
    %>
		<br />&nbsp;<a href="<%=backurl%>">返回上一级>></a><br /> &nbsp;<a
			href="/wap/goods.jsp?productid=<%=goodsid %>">返回商品详情>></a>
		<div style="background: #FFDEAD; padding: 3px; width: 100%;">
