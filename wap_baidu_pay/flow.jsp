...
<body>
	...
	<div style="margin-bottom: 15px;">
		<div style="background: #FFDEAD; padding: 3px; width: 100%;">
			<a href="/mindex.jsp">首页</a>>购物车 <br />
		</div>
		<span style="color: #f00"><%= infos %></span><br />
		<div>
			<font color='#ff0000'>注：如需修改商品数量，填写好数字后请点击“修改”按钮。</font>
		</div>
	...
	if(cartList!=null&&cartList.size()>0)
	{
		for(Cart cart : cartList){
			...
			<tr id="cart_<%= rec_key%>">
				<td colspan="2">&nbsp;<a
					href="/wap/goods.jsp?productid=<%= product.getId() %>"><%= Tools.clearHTML(product.getGdsmst_gdsname()) %></a>
					<%if(Tools.longValue(product.getGdsmst_specialflag()) == 1){
      					%> &nbsp;<span style="color: red;">该商品不能使用优惠券</span> <%}%></td>
			</tr>
			<% if(product.getGdsmst_skuname1()!=null&&product.getGdsmst_skuname1().length()>0)
		       {%>
			<tr id="cart1_<%= rec_key%>">
				<td>&nbsp;<%= product.getGdsmst_skuname1() %>：
				</td>
				<td><%= sku!=null?sku.getSkumst_sku1():"无"  %></td>
			</tr>
			<%}
		       %>
			<tr id="cart2_<%= rec_key%>">
				<td>&nbsp;会员价：</td>
				<td><%= price %>元</td>
			</tr>
			<tr id="cart3_<%= rec_key%>" style="border-bottom: solid 1px #333;">
				...
				<td>
					<%if(hasFather || type==0 || type==13 || type==-5 ||type==14|| type==2||(product!=null&&product.getGdsmst_buylimit()!=null&&product.getGdsmst_buylimit().intValue()>0)){
		              %><%=amount %> <%}else{%>
					<form id="u_<%= product.getId() %>" method="post"
						action="flow.jsp?act=u&pid=<%= product.getId() %>&carid=<%= id%>&rec_key=<%= rec_key%>">
						<input type="hidden" id="p_<%= product.getId() %>"
							name="p_<%= product.getId() %>" value="<%= product.getId() %>" />
						<input type="text" style="width: 50px;" value="<%= amount%>"
							attr="<% if(product.getGdsmst_buylimit()!=null&&product.getGdsmst_buylimit().longValue()>0){ out.print(product.getGdsmst_buylimit().longValue());} else
		        out.print("0");	       %>"
							id="c_<%= product.getId() %>" name="c_<%= product.getId() %>" />
						...
						&nbsp;&nbsp;
						<a
							href="updateflow.jsp?type=<%= type %>&rec_key=<%= rec_key%>&is_child=<%=Tools.longValue(cart.getHasChild()) %>"
							id="del_<%=rec_key %>"">删除</a>
    ...
    %>
			<tr>
				<td colspan="2">&nbsp;<span id="total" style="color: #f00"><font
						style="color:#d80100; font-size:16px; font-weight:bold;">应付总额：<%=Tools.getFormatMoney(totalPrice) %>元</span></td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;
					<form method="post" action="flow.jsp?act=js">
						<input type="hidden" name="guid"
							value="<%if(lUser!=null){%> <%=lUser.getId()%><%}else{ %><%}%>" />
						<input id="checkout" type="submit" value="去结算"
							style="padding: 3px;" />
					</form>
				</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;<a href="/mindex.jsp">继续购物>></a></td>
			</tr>
			<%}
	else{
		...
		<table>
			...
			<% for(int i=0,j=0;i<size&&j<5;i++){
					...
					String id = product.getId();
								  %>
			<tr>
				<td><a href="/wap/goods.jsp?<%=product.getId()%>"><%=title %></a></td>
			</tr>
			<%
					    ...
					    %>
			<tr>
				<td><%= product.getGdsmst_skuname1() %>：<%= sku.getSkumst_sku1() %></td>
			</tr>
			<%}
					    }
					%>
			<tr>
				<td>会员价：￥<%= product.getGdsmst_memberprice().longValue()%></td>
			</tr>
			<tr>
				<td class="othertd"><a
					href="<%  
					...
					else out.print("/wap/goodsend.jsp?gdsid="+product.getId()+"&skuId="+c57.getSkuId());%>">加入购物车</a>
				</td>
			</tr>
			<%
									j++;
