...
<body>
    ...
    %>
		<div style="background: #FFDEAD; padding: 3px; width: 100%;">
			收货地址管理 | <a
				href="/wap/user/addressprovince.jsp?op=add&<%=url %>&from=flow">添加</a>
			<br />
		</div>
	...
	if(list!=null&&list.size()>0){
		...
		for(UserAddress ua:list){
			...
			<input type="radio" name="address" value="<%= ua.getId()%>"
				<% if(ua.getId().equals(addressid)) out.print("checked");%> /><%=ua.getMbrcst_raddress() %>
			<a href="/wap/flowCheck1.jsp?<%=url %>&addressid=<%=ua.getId()%>">确定</a>&nbsp;&nbsp;
			<a
				href="/wap/user/addressprovince.jsp?<%=url %>&addressid=<%=ua.getId()%>&id=<%=ua.getMbrcst_provinceid()%>&from=flow">修改</a>&nbsp;&nbsp;
			<a
				href="flowCheck_op.jsp?<%=url %>&addressid=<%=ua.getId()%>&deladdress=1">删除</a>
			<br />
			<%
			}
		}%>
	...
	{%>
			您还没有收货地址，
			<a href="/wap/user/addressprovince.jsp?op=add&<%=url %>&from=flow">马上去添加</a>
			<%}
    %>
			<p>
