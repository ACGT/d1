...
%>
	...
	<form action="flowCheck1.jsp" method="post">
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td>收货人：<%=uaddress.getMbrcst_name() %></td>
			</tr>
			<tr>
				<td>收货地址：<%=uaddress.getMbrcst_raddress() %> 邮编：<%=uaddress.getMbrcst_rzipcode() %>
				</td>
			</tr>
			<tr>
				<td>联系方式：<%=uaddress.getMbrcst_rphone() %> 邮箱地址：<%=uaddress.getMbrcst_remail() %></td>
			</tr>
			<tr>
				<td height="5px"></td>
			</tr>
			<tr>
				<td><a href="/wap/flow/addresseelist.jsp?<%=url%>">修改>></a></td>
			</tr>
		</table>
		<%} %>
		...
		<table border="0" cellpadding="0" cellspacing="0" id="tblPayHead"
			...
			<tr>
				<td class="d2"><span class="t23">&nbsp;支付方式
						(北京地区支持POS机刷卡)</span></td>
			...
			<tr>
				<td><input type="radio" name="req_payid" value="0"
					<%if(payid.equals("0")){ %> checked="checked" <%} %>>现金支付（货到付款）</input></td>
			</tr>
			<%
			...
			<%//}
        		...
        		%>
			<tr>
				<td><input type="radio" name="req_payid" value="20"
					<%if(payid.equals("20")){ %> checked="checked" <%} %>>支付宝支付(在线支付)</input></td>
			</tr>
			<!-- 
        		<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<tr>
				<td>&nbsp;<a
					href="/wap/flowCheck1.jsp?<%=url%>&cancelticket=1">取消使用该优惠券</a>
				</td>
			...
			<tr>
				<td>&nbsp;该优惠券不可用。&nbsp;<a
					href="/wap/flow/ticket.jsp?<%=url%>">使用优惠券</a>
				</td>
			</tr>
			<%}
			...
			<%}
            	...
            	if(Tools.isNull(IsUsePrepay)){
            		...
            		%>
			<tr>
				<td>&nbsp;<a href="/wap/flow/prepay.jsp?<%=url %>">使用预存款支付</a>
				</td>
			</tr>
			<%}else{%>
			<tr>
				<td>&nbsp;已使用<%=usemoney%>元预存款
				</td>
			</tr>
			<tr>
				<td>&nbsp;<a href="/wap/flowCheck1.jsp?<%=url%>&cancelprepay=1">取消使用预存款</a>
				</td>
			</tr>
			<%}
            	}
								...
								<table width="200" border="0" cellpadding="0" cellspacing="5">
									...
									<tr>
										...
										<td align="left"><span class="t66"><span
												id="lblTotal"><%=Tools.getFloat(yingfu,2) %></span>元</span></td>
									</tr>
									<tr>
			...
			<tr>
				<td align="center">
					<div id="divBtnOrder" style="display: block; text-align: center;">
						&nbsp;<input type="submit" id="Submit33" name="Submit33"
							class="SubmitOrder" value="提交订单" />
					</div>
		...
		<input name="ticketid" type="hidden"
			value="<%=request.getParameter("ticketid")%>"></input>
