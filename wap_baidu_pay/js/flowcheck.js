	  ...
	  $.ajax({
		...
		success: function(json){
		 if(json.status=="-1"){
			 ...
			 var payhtml='<span><input class="pay" type="radio" checked id="payid1" name="payid" onclick="loadprice();" value="20">支付宝支付</span>';
			 payhtml ＋＝ '<span><input class="pay" type="radio" checked id="payid3" name="payid" onclick="loadprice();" value="30">百度支付</span>';
			 if(json.ifCanHF){
				 payhtml+='<span><input class="pay" type="radio" id="payid2" name="payid" onclick="loadprice();" value="0">货到付款</span>';
			 }
			 $(".m_pay .item").append(payhtml);
