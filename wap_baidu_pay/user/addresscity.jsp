...
<%  
	...
	{
		id=request.getParameter("hideProvId").trim();
	}
	String oldpid="";
	String oldcityid="";
	String addressid="";
	if(!Tools.isNull(request.getParameter("hideAddressId")))
	{
		...
		if(address!=null){
			oldcityid=address.getMbrcst_cityid().toString();
			oldpid=address.getMbrcst_provinceid().toString();
		}
	}
	if(!Tools.isNull(request.getParameter("addressid")))
	{
	   ...
	   String title="修改收货地址";
	    ...
	    if("post".equals(request.getMethod().toLowerCase()) ){
	    	if(!Tools.isNull(request.getParameter("ddlCity")) && !"0".equals(request.getParameter("ddlCity")))
	    	{
	    		response.sendRedirect("addressupdate.jsp?"+url+"&hideAddressId="+addressid+"&hideProvId="+id+"&hideCityId="+request.getParameter("ddlCity")+"&op="+op);
...
<body>
	...
	<div style="margin-bottom: 15px;">
     ...
     %>
		...
		<form action="addresscity.jsp" method="post">
			<input id="hideProvId" name="hideProvId" type="hidden"
				value="<%=id %>" />
			<input id="hideCityId" name="hideCityId" type="hidden"
				value="<%=oldcityid %>" />
			...
			<div id="second" style="margin-top: 2px; line-height: 18px;">
				<span style="color: red;"><%=errormsg %></span> <br /> 第2步/共3步<br />
				<span id="citytip" style="color: red;"></span> &nbsp;<font
					color='red'>*</font>&nbsp;县/市/区 <select id="ddlCity" name="ddlCity">
					<option value="0">==请选择==</option>
					<%
            ArrayList<City> list = CityHelper.getCitysViaProvinceId(id);
        	if(list != null && !list.isEmpty()){
        		StringBuilder sb = new StringBuilder();
        		for(City city : list){
        			%>
					<option value="<%=city.getId()%>"
						<%if(id.equals(oldpid) && city.getId().equals(oldcityid)){ %>
						selected="selected" <%} %>><%=city.getCtymst_name()%></option>
					<%}
        	}
            %>
				</select><br /> <input type="submit" value="下一步"
					style="width: 65px; padding: 4px;" />&nbsp;&nbsp;<a
					href="<%=backurl+"?"+url %>" style="text-decoration: underline">返回上一步</a>
			</div>
