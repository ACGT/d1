...
%>
	...
	<div style="margin-bottom: 15px;">
		...
		<div style="padding: 5px;">
			<a href="comment.jsp">待评价</a>&nbsp;&nbsp;&nbsp;&nbsp;<a
				href="comment.jsp?type=1">已评价</a>
			<%
     if(Tools.isNull(request.getParameter("type")))//待评价
     {
    	 ArrayList<OrderBase> nocommentlist=OrderHelper.getMyFinishOrderList(lUser.getId()) ;
			...
			<table border="0" cellspacing="1" cellpadding="0" class="t">
				...
				<tr>
					<td width="100"><a
						href="/wap/orderdetail.jsp?orderid=<%=base.getId() %>"
						target="_blank"><%=base.getId() %></a></td>
					<td><a
						href="/wap/comment/addcomment.jsp?orderid=<%=base.getId() %>"
						target="_blank">确认收货并评价</a></td>
				</tr>
			...
			<%} else{%>
			<font color='#f00'>您目前没有待评价的订单！</font><br /> <a href="/wap/user">返回我的优尚</a>
			<%} }else{
	   %>
			   ...
			   for(Comment cache:list){
				   int i=0;
		 			  ...
		 			  String url="";
						 ...
						 int score=cache.getGdscom_level().intValue();
							...
							%>
				<tr>
					<td style="color: #a25663;" width="80">订单号</td>
					<td width="80"><a
				...
				<tr>
					...
					<td><span class="sptitle" style="display: block;"><a
							href="<%=url %>" target="_blank"><%=cache.getGdscom_gdsname() %></a></span></td>
				</tr>
				<tr>
	    ...
	    if(cache.getGdscom_replyStatus().intValue()==1 || !Tools.isNull(cache.getGdscom_replyContent())){
	    	%>
              ...
              if (currentPageIndex>1 ){%>
							<a href="javascript:gopage2(<%=currentPageIndex-1%>)"><font
