       ...
       else
       {
    	   ...
    	   if(asklist!=null&&asklist.size()>0)
    	   {
    		   ...
    		   for(int i=10*(pageno-1);i<pageno*10&&i<asklist.size();i++)
               {
            	   ...
            	   if(ga!=null)
            	   {%>
            		 <tr><td style=" text-align:right">用户：</td><td><%out.print(ga.getGdsask_uid()); %></td>
            		 <td style=" text-align:right"><%= new SimpleDateFormat("yyyy-MM-dd HH:mm").format(ga.getGdsask_createdate()) %></td></tr>  
            		 <tr><td style=" text-align:right">问题：</td><td colspan="2"><%=Tools.clearHTML( ga.getGdsask_content()) %></td></tr>
            		 <tr style=" color:#f00; border-bottom:solid 1px #333; "><td>&nbsp;D1回复：</td><td colspan="2"><%= ga.getGdsask_replyContent() %></td></tr>
            	   <%}
               }
    		   out.print("</table>");  
    		   ...
    		   &nbsp;<a href="<%=request.getRequestURI()%>?productid=<%=goodsid %>&pageno=<%= pageno==pagecount?pageno:pageno+1%>">下页</a>
    		   <input type="text" style="width:50px;" id="page" name="pageno" value="<%=pageno%>"/>/<%=pagecount  %>   <input type="submit" value="跳&nbsp;转" style=" padding:4px;" />
    		   <br/>&nbsp;<a href="<%=request.getRequestURI()%>?productid=<%=goodsid %>&pageno=<%= pageno==1?pageno:pageno-1%>">返回上一页>></a><br/>
