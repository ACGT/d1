...
<body >
   ...
   <div style=" background:#FFDEAD; padding:3px; width:100%;">
    <a href="/mindex.jsp">首页</a>>新品
    <br/>
    </div>
    <div>热搜新品：<a href="/wap/new/index.jsp?">全部</a></div>
    <table>
      <tr><td width="60"><A href="index.jsp?productsort=014001" <%if(rackcode.equals("014001")){%> class="sele" <%}%> >护肤品</A></td>
      <td width="50"><A href="index.jsp?productsort=014003" <%if(rackcode.equals("014003")){%> class="sele" <%}%> >彩妆</A></td>
      <td><A href="index.jsp?productsort=014002" <%if(rackcode.equals("014002")){%> class="sele" <%}%> >香水</A></td></tr>
      <tr><td><A href="index.jsp?productsort=020" <%if(rackcode.equals("020")){%> class="sele" <%}%> >女装</A></td>
      <td><A href="index.jsp?productsort=021" <%if(rackcode.equals("021")){%> class="sele" <%}%> >女鞋</A></td>
      <td><A href="index.jsp?productsort=023" <%if(rackcode.equals("023")){%> class="sele" <%}%> >女包</A></td></tr>
      <tr><td><A href="index.jsp?productsort=015009,015002014" <% if(rackcode.equals("015009")||rackcode.equals("015002014")){%> class="sele" <%}%>>饰品</A></td>
      <td><A href="index.jsp?productsort=030" <%if(rackcode.equals("030")){%> class="sele" <%}%>>男装</A></td>
      <td><A href="index.jsp?productsort=015002004" <%if(rackcode.equals("015002004")){%> class="sele" <%}%>>名表</A></td></tr>
      <tr><td><A href="index.jsp?productsort=033" <%if(rackcode.equals("033")){%> class="sele" <%}%>>皮具</A></td>
      <td><A href="index.jsp?productsort=014001018" <%if(rackcode.equals("014001018")){%> class="sele" <%}%>>男士护肤</A></td></tr>
    </table>
	        	    ...
	        	    if(pBean.getTotalPages()>=1)
	        	    {
	        	    	%>
	        	          <tr><td colspan="3">
	        	            ...
	        	            <a href="<%=pageURL%>pageno=<%= pBean.getCurrentPage()==1?1:pBean.getPreviousPage()%>">上一页</a>
	        	            <a href="<%=pageURL%>pageno=<%= pBean.getCurrentPage()==pBean.getTotalPages()?pBean.getTotalPages():pBean.getNextPage()%>">下一页</a>
	        	            <form id="pagess" action="<%=pageURL%>" method="post">
	        	               <input type="text" id="page" name="page" style=" width:50px;" value="<%=pBean.getCurrentPage()%>"/>/<%=pBean.getTotalPages() %>页
