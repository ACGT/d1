...
<div class="all">
     <div class="box">
				...
				for(Directory dir : dirList){
					%><li><a href="/wap/resultlist.jsp?productsort=<%=dir.getId() %>"><%=dir.getRakmst_rackname() %>(<%=((ProductManager)Tools.getManager(Product.class)).getRackcodeProductLength(dir.getId())%>)</a></li><%
					...
					if(childDirList != null && !childDirList.isEmpty()){
						for(Directory childDir : childDirList){
							%><li class="newli"><a href="/wap/resultlist.jsp?productsort=<%=childDir.getId() %>"><%=childDir.getRakmst_rackname() %>(<%=((ProductManager)Tools.getManager(Product.class)).getRackcodeProductLength(childDir.getId())%>)</a></li><%
						}
					}
				}
				%></ul></div><%
