...
<div>
    ...
    <a href="/mindex.jsp">首页</a>>搜索结果
    <br/>
     </div>
    <table>
    <tr><td>&nbsp;在购物中搜索<font style="color:#f00"><%=keyWords %></font>共找到<font style="color:#f00"><%=sr.getTotalcount(rackcode)%></font>个结果</td></tr>
    <tr><td>&nbsp;筛选：<%
		    ...
		    if(list123!=null&&list123.size()>0){
		    	if(list123!=null){
					...
					%>
                             ...
                             <a href="<%=request.getRequestURI()%>?key_wds=<%=Base64.encode(keyWords)%>&rackcode=<%=s123%>"><%=dir.getRakmst_rackname()%></a> <span style="color:#aa2e44">(<%=sr.getTotalcount(s123)%>)</span>&nbsp;&nbsp;             
                        
                    <%}}}//end if
                  }%>
              </td></tr>
     <tr><td>&nbsp;排序：<a href="<%=request.getRequestURI()%>?key_wds=<%=Base64.encode(keyWords)%>&sort=createtime&asc=false&<%if(!Tools.isNull(rackcode))out.print("rackcode="+rackcode);%>" >最新上架</a>
            	<a href="<%=request.getRequestURI()%>?key_wds=<%=Base64.encode(keyWords)%>&sort=sales&asc=false&<%if(!Tools.isNull(rackcode))out.print("rackcode="+rackcode);%>" >热销排行</a>
                <a href="<%=request.getRequestURI()%>?key_wds=<%=Base64.encode(keyWords)%>&sort=price&asc=false&<%if(!Tools.isNull(rackcode))out.print("rackcode="+rackcode);%>" >价格↓</a>
                <a href="<%=request.getRequestURI()%>?key_wds=<%=Base64.encode(keyWords)%>&sort=price&asc=true&<%if(!Tools.isNull(rackcode))out.print("rackcode="+rackcode);%>" >价格↑</a>
                </td></tr>
    </table>
    <br/>
    <table>
    ...
    if(size>0)
    {
    	for(Product p:list)
    	{
    		...
    		%>
    		    <tr><td><a href="/wap/goods.jsp?productid=<%= p.getId() %>"><%= Tools.clearHTML(p.getGdsmst_gdsname()) %></a></td></tr>
				<tr><td><a href="/wap/goods.jsp?productid=<%= p.getId() %>"><img src="<%= theimgurl%>" width="120" height="120"/></a></td></tr>
    			<tr><td><%
    			...
    			%></td></tr>
				   <tr><td  style=" border-bottom:solid 1px #ccc;"><span style="float:left;">顾客评分：</span><span class="sa<%=score %>" style="float:left;" ></span>(已有<a href="/wap/comment/commentlist.jsp?productid=<%= p.getId() %>"><%= comcount%></a>人评价)</td></tr>
    		<%}
    	}
    }
    %>
    ...
    <tr><td>
              <a href="<%=request.getRequestURI() %>?<%=pgQueryString%>pg=<%=pb.getCurrentPage()==1?1:pb.getPreviousPage() %>" class="pag1">上页</a> &nbsp;
			  <a href="<%=request.getRequestURI() %>?<%=pgQueryString%>pg=<%=pb.getCurrentPage()==pb.getTotalPages()?pb.getTotalPages():pb.getNextPage() %>" class="pag1">下页</a> &nbsp; 
			  <form id="rpage" action="<%=request.getRequestURI() %>?<%=pgQueryString%>" method="post">
			      <input id="page" name="page" type="text" style="width:50px;" value="<%=pb.getCurrentPage() %>"/>/<%= pb.getTotalPages() %>页
				  <input type="submit" value="跳&nbsp;转" style="padding:4px;" /><br/>
...
<a href="">购物车</a>&nbsp;&nbsp;<br/>
<a href="/wap/login.jsp">登录</a>&nbsp;&nbsp;<a href="/wap/regist.jsp">注册</a>&nbsp;&nbsp;<a href="/wap/html/help.jsp">帮助</a>
 <br/>
