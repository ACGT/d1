...
<%
if (1==1){
	response.sendRedirect("/wap/user/orderlist.html");
	return;
}
%>
...
<title>D1优尚网-会员专区—四个月之内订单</title>
<style type="text/css">
body{ line-height:21px;background:#fff; padding-left:4px; color:#333 }
...
<div style=" margin-bottom:15px;">
   <div style=" background:#FFDEAD; padding:3px; width:100%;">
    <a href="/mindex.jsp">首页</a>><a href="index.jsp">我的优尚</a>>我的四个月订单
    <br/>
    </div>
    <% 
    String msg="";
...
//翻页
  int totalLength1 =(result != null ?result.size() : 0);
    ...
    if(result==null||result.size()==0){
    	    ...
    	    out.print("<br/><font color='red'>您四个月之内没有下过订单!</font><a href=\"/wap/user/myorder_history.jsp\">请查看四个月前订单</a>");
    	  }
    else
    {
    	...
    	for(int i=(pageno1-1)*5;i<pageno1*5&&i<result.size();i++)
    	{
    	   ...
    	   {
    	%>
    		<tr><td align="right">订单号：</td><td><a href="orderdetail.jsp?orderid=<%= ob.getId() %>"><%= ob.getId() %></a></td></tr>
    		<tr><td>订购时间：</td><td><%= new SimpleDateFormat("yyyy-MM-dd HH:mm").format(ob.getOdrmst_orderdate()) %></td></tr>
    		<tR><td>应付金额：</td><td><%= OrderHelper.getOrderTotalMoney(ob.getId()) %>元 </td></tR>
    		<tr><td>订单状态：</td><td><%= getOrderStatuByPaytype(ob.getOdrmst_paytype().toString(),ob.getOdrmst_orderstatus().toString())   %></td></tr>
    		<tr ><td colspan="2" align="left" style="border-bottom:solid 1px #00f;">
    		<% if(ob.getOdrmst_orderstatus().longValue()>0) {
    		   ...
    		   %>
    			...
    			if(ob.getOdrmst_orderstatus().longValue()==5||ob.getOdrmst_orderstatus().longValue()==51||ob.getOdrmst_orderstatus().longValue()==61||ob.getOdrmst_orderstatus().longValue()==6)
    			{
    			    ...
    			    {
    			 %>
    				<a href="/wap/comment/addcomment1.jsp?orderid=<%= ob.getId() %>">我要评论>></a>
    			<%  }
    			...
    			{%>
    				<a href="/wap/comment/addcomment1.jsp?orderid=<%= ob.getId() %>">确认收货并评价>></a>
    			<%}
    			else
    			{
    				if(ob.getOdrmst_payid().longValue()==20){//支付宝手机支付
    					%>
    					<a href="/interface/pay/wapalipay/Trade.jsp?OdrID=<%= ob.getId() %>">去支付>></a><br/>
    			<%	}
	   ...
	   //分页
	    ...
	    if(pBean1.getTotalPages()>1){%>
	     <form action="myorder.jsp">
	    	<br/><a href="<%=pageURL1%>pageno1=<%=pBean1.getNextPage()==-1?pBean1.getTotalPages():pBean1.getNextPage()%>">下页</a>
			<input type="text" style=" width:20px;" id="page" name="pageno1" value="<%=pageno1%>"/>/<%=pBean1.getTotalPages() %>页<input type="submit" value="跳&nbsp;转"  style=" width:80px; height:20px;"></input>
			<br/><a href="<%=pageURL1%>pageno1=<%=pBean1.getPreviousPage()==-1?1:pBean1.getPreviousPage()%>">返回上一页>></a>		       
	   </form>
	    <%}
	    %>
