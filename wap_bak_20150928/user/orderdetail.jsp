<%@ page contentType="text/html; charset=UTF-8" import="net.sf.json.*"%><%@include file="/inc/header.jsp"%>
...
<div style=" margin-bottom:15px;">
   <div style=" background:#FFDEAD; padding:3px; width:100%;">
    <a href="/mindex.jsp">首页</a>><a href="index.jsp">我的优尚</a>>订单详情
    <br/>
    </div>
      &nbsp;订单号：<%=orderid %>
   <table  width="100%">
      <tr><td colspan="2" style=" background:#FFDEAD; color:#f00;">&nbsp;收货信息</td></tr>
      <tr><td align="left" width="100">&nbsp;收货人：</td><td> <%=ob.getOdrmst_rname()%> </td></tr>
      <tr><td align="left">&nbsp;收货地址：</td><td><%=ob.getOdrmst_rprovince()%>，<%=ob.getOdrmst_rcity()%>，<%=ob.getOdrmst_raddress()%>，<%=ob.getOdrmst_rzipcode()%></td></tr>
      <tr><td align="left">&nbsp;联系方式：</td><td>手机&nbsp;<% if(ob.getOdrmst_rphone()!=null&&ob.getOdrmst_rphone().length()>0){ out.print(ob.getOdrmst_rphone());}
					     else
					     { out.print(ob.getOdrmst_pmphone());}
					     %>，邮箱地址&nbsp;<%=ob.getOdrmst_remail()%></td></tr>
	    <tr><td colspan="2" style=" background:#FFDEAD; color:#f00;">&nbsp;送货方式</td></tr>
	    <tr><td colspan="2">&nbsp;
	    <%if(!Tools.isNull(ob.getOdrmst_customerword())){ %>
	    ...
	    <%
           ...
           %>
	    	   ...
	    	   <%
	    	      ...
	    	      %>
	    	    	<tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_orderdate())%>&nbsp;&nbsp;订单生成</td></tr>
	      ...
	      <%
           ...
           <!--订单状态2--用户取消-->
       			  ...
       			  <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_orderdate())%>&nbsp;&nbsp;订单生成</td></tr>
       		      <tr><td colspan="2"><%	if (ob.getOdrmst_canceldate() != null){
           ...
           <!--订单状态1--货到付款已确认-->
	                   <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_orderdate())%>&nbsp;&nbsp;订单生成</td></tr>
       		      <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_validdate()) %>&nbsp;&nbsp;<%=payType==4?"已成功支付":"订单已审核" %></td></tr>	
		...
		<!--订单追踪-->	  
			  ...
			  %>
			      <tr><td colspan="2" align="left"><%=dateFormat.format(ob.getOdrmst_orderdate())%>&nbsp;&nbsp;订单生成</td></tr>
       		      <tr><td colspan="2" align="left"><%=dateFormat.format(ob.getOdrmst_validdate()) %>&nbsp;&nbsp;<%=payType==4?"已成功支付":"订单已审核" %></td></tr>	
				  <tr><td colspan="2" align="left"><%=dateFormat.format(ob.getOdrmst_shipdate())%>&nbsp;&nbsp;备货完成</td></tr>	
				  <tr><td colspan="2" align="left"><%=dateFormat.format(ob.getOdrmst_realshipdate())%>&nbsp;&nbsp;快递配送</td></tr>
				  <tr><td colspan="2"> <%
					   ...
					   %>
						...
						<!-- 加物流提示开始 -->
						   <a href="/wap/user/orderlogistics.jsp?orderid=<%=orderid%>" title="物流信息">物流信息查询</a><br/>	
						<!-- 加物流提示结束 -->
					   <%
					   } %>
					   商品收到了，请您尽快<a href="/wap/comment/addcomment1.jsp?orderid=<%=orderid%>">确认收货并评价</a>，您的评价对其他客户购买商品有很大的帮助！！！
				  </td></tr>		
            ...
            <%
                ...
                %>
				  ...
				  <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_orderdate())%>&nbsp;&nbsp;订单生成</td></tr>
       		      <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_validdate()) %>&nbsp;&nbsp;<%=payType==4?"已成功支付":"订单已审核" %></td></tr>	
				  <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_shipdate())%>&nbsp;&nbsp;备货完成</td></tr>	<%
            }
            	break;
            		}
            ...
            %>
			  ...
			  <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_orderdate())%>&nbsp;&nbsp;订单生成</td></tr>
       		      <tr><td colspan="2"><%=dateFormat.format(ob.getOdrmst_validdate()) %>&nbsp;&nbsp;<%=payType==4?"已成功支付":"订单已审核" %></td></tr>	
				  <tr><td colspan="2"><%=ob.getOdrmst_shipdate() == null ? "" : dateFormat.format(ob.getOdrmst_shipdate())%>&nbsp;&nbsp;备货完成</td></tr>	
				  <tr><td colspan="2"><%=ob.getOdrmst_realshipdate() == null ? "" : dateFormat.format(ob.getOdrmst_realshipdate())%>&nbsp;&nbsp;快递配送 </td></tr>	
				  <tr><td colspan="2"> <%
					   if(!Tools.isNull(d1shipmethod)){
					   %>
					   货单号：<%=Tools.formatString(ob.getOdrmst_goodsodrid()) %>
			  ...
			  <tr><td colspan="2"><% if (ob.getOdrmst_finishdate() != null) {
			  	...
			  	<%
					    comment = getCommentbyOrderId(ob.getId());
			  	        if(comment==null)
			  	        {%>
			  	        	&nbsp;&nbsp;<a href="/wap/comment/addcomment1.jsp?orderid=<%= ob.getId() %>">马上去评价</a>
			  	        <%}
			  	%>
			  	</td></tr>
					    ...
					    if(comment != null){
					    %>
					    	<tr><td colspan="2"><%=dateFormat.format(comment.getGdscom_createdate()) %>&nbsp;&nbsp;已评价</td></tr>
					    	<%
					    }%>
...
if(ob.getOdrmst_payid().intValue()==0){
	...
	%>
 <tr><td colspan="2" style=" background:#FFDEAD; color:#f00;">&nbsp;付款方式</td></tr>
 <tr><td colspan="2">&nbsp; <%=paymethod%>
 <%
 if(ob.getOdrmst_payid().intValue()==20 && ob.getOdrmst_orderstatus().longValue()==0){
	 %> 
	 <br/><a href="/interface/pay/wapalipay/Trade.jsp?OdrID=<%=orderid%>">去支付>></a>
  <%}
 %>
 </td></tr>
 
  <tr><td colspan="2" style=" background:#FFDEAD; color:#f00;">&nbsp;顾客留言</td></tr>
  <tr><td colspan="2">&nbsp;<%=memo%></td></tr>
  <tr><td colspan="2" style=" background:#FFDEAD; color:#f00;">&nbsp;商品清单</td></tr>
  <tr><td colspan="2"><%
				     	oilist = OrderItemHelper.getOdrdtlListByOrderId(orderid);
						     ...
						     %>
						        	...
						        	<br/>
						        	<% if(oi.getOdrdtl_sku1()!=null&&oi.getOdrdtl_sku1().length()>0&&p.getGdsmst_skuname1()!=null&&p.getGdsmst_skuname1().length()>0)
						        		{
             ...
             <tr><td>&nbsp;商品金额：&nbsp;</td> <td>￥<%= OrderHelper.getOrderTotalProductMoney(orderid) %>元</td></tr>
             <tr><td>&nbsp;+&nbsp;运费：&nbsp;</td> <td>￥<%=OrderHelper.getOrderExpressFee(orderid) %>元</td></tr>
             <tr><td>&nbsp;-&nbsp;优惠：&nbsp;</td> <td>￥<%= OrderHelper.getOrderTotalCut(orderid) %>元</td></tr>
              <% if(OrderHelper.getOrderTotalCut1(orderid)>0f)
		               {
		            	   out.print("<tr><td>&nbsp;-&nbsp;预存款：&nbsp;</td><td>￥"+OrderHelper.getOrderTotalCut1(orderid)+"元</td></tr>");
		               }
		     %>
		     <tr><td>订单总金额：</td> <td>￥<%=OrderHelper.getOrderTotalMoney(orderid) %>元</td></tr>
		     		<tr><td colspan="2"><%
