...
<form id="form1" name="form1" action="operation.jsp?orderid=<%=request.getParameter("orderid") %>" method="post"  >
      ...
      if(!Tools.isNull(request.getParameter("orderid"))){
    	  ...
    	  <tr><td  height="5">&nbsp;</td></tr>
			<tr> <td  class="peisong_body" align="left"  style=" padding-left:5px; padding-bottom:5px">
         ...
         <td  class="peisong_body" align="left" valign="middle" style="padding-left:4px; padding-right:4px">
          <div style="background-color:#FEEEF1;"><span style="font-size:12px; ">感谢您的参与，成功评价1个商品，您将获得10个积分（</span><span style="font-size:12px; color:red; ">平安万里通用户除外</span><span style="font-size:12px; ">）。</span></div>
        ...
        <div style="padding-left:4px; margin-left:0px; float:left">
		    ...
		    <input type="radio" value="5" name="rlevel<%=i%>"  onclick="check<%=i%>(5)">5星</input>
		     <input type="radio" value="4" name="rlevel<%=i%>"  onclick="check<%=i%>(4)">4星</input>
		      <input type="radio" value="3" name="rlevel<%=i%>"  onclick="check<%=i%>(3)">3星</input>
		       <input type="radio" value="2" name="rlevel<%=i%>"  onclick="check<%=i%>(2)">2星</input>
		        <input type="radio" value="1" name="rlevel<%=i%>"  onclick="check<%=i%>(1)">1星</input>
         
          </div>
          </div>
           </td>
          </tr>
          <tr><td style="padding-left:4px">请输入评论内容（100字以内）：</td></tr>
          <tr><td style="padding-left:4px"><textarea class="txtcontent" name="tcontent" id="tcomment<%=i %>"   onkeydown='if (this.value.length>=100){event.returnValue=false}' onkeyup="keypress<%=i%>(this.value)"></textarea>
          <br/>
...
<tr><td align="center"><input type="button" onclick="add();" value="提交"></input></td></tr>
            </table>
