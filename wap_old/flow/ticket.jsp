...
%><table border="0"  cellpadding="2" cellspacing="2">
		...
		if(Tools.floatCompare(m_fltBrdTktPrice,0)==1){
			...
			<tr>
				<td  class="t00"><a href="/wap/flowCheck1.jsp?ticketid=brdtkt<%=url %>>">使用该优惠券</a></td>
			</tr>
			<%
		}
		if(list1 != null && !list1.isEmpty()){
			for(Ticket ticket : list1){
				...
				<tr>
					...
					<tr>
				<td  class="t00"><a href="/wap/flowCheck1.jsp?ticketid=<%=ticket.getId()%>&ticketvalue=<%=Tools.floatValue(ticket.getTktmst_value()) %><%=url %>">使用该优惠券</a></td>
			</tr>
				<%
			}
		}
		if(list2 != null && !list2.isEmpty() && CartHelper.getNormalProductMoney(request, response)>0){
			for(TicketCrd ticket : list2){
				...
				%><tr>
					...
					<tr>
				<td  class="t00"><a href="/wap/flowCheck1.jsp?ticketid=crd<%=ticket.getId()%>&ticketvalue=<%=iTkt_value %><%=url %>">使用该优惠券</a></td>
			</tr>
					<%
			}
		}
...
%>
     ...
     <div style=" background:#FFDEAD; padding:3px; width:100%;">激活新优惠券
    <br/>
    </div>
    <%
    String msg="";
