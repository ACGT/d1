...
body{ background:#fff;font:14px Arial,"微软雅黑";color:#4b4b4b; padding-bottom:15px; line-height:18px;  margin-left:5px; width:100%;}
...
<div style=" margin-bottom:15px;">
   <div style=" background:#FFDEAD; padding:3px; width:100%;">
			...
			if(!Tools.isNull(rackCode)){
				...
				if(size >= 3){
					for (int i = 3; i <= size; i = i + 3){
						...
						if(i==3){
							%>><a href="/wap/resultlist.jsp?productsort=<%=directory.getId() %>"><%=category %></a><%
						}else{
							%>><a href="/wap/resultlist.jsp?productsort=<%=directory.getId() %>"><%=category %></a><%
						}
					}
    ...
    <div style="background:#f00; color:#fff;">&nbsp;<a href="/wap/goods.jsp?productid=<%= id %>"  style="color:#fff;"><b>简介</b></a>&nbsp;|&nbsp;<a href="/wap/imglist.jsp?productid=<%=id %>" style="color:#fff;"><b>图片</b></a>&nbsp;|&nbsp;
     <a href="/wap/goodsdetail.jsp?productid=<%= id %>" style="color:#fff;"><b>详情</b></a>&nbsp;|&nbsp;<a href="/wap/comment/commentlist.jsp?productid=<%= id%>" style="color:#fff;"><b>评论(<%=CommentHelper.getCommentLength(id)%>)</b></a>&nbsp;</div>
   <table>
     
     <tr><td>&nbsp;<b><%= product.getGdsmst_gdsname() %><%if(Tools.longValue(product.getGdsmst_specialflag()) == 1){
      					%>	
     ...
     if(product.getGdsmst_recimg()!=null) {
    	 ...
    	 out.print(theimgurl);
    		 %>" width="160" height="160"/></td></tr>
    	 
     <tr><td>&nbsp;商品编号：<%= product.getId() %></td></tr>
     <% if(msflag){
     ...
     <tr><td>&nbsp;<b>基本信息：</b></td></tr>
     <tr><td style=" border-bottom:solid 1px #333;"><%
						...
						<%
									    ...
									    <%
									       ...
									       {%>
									    	   &nbsp;<a href="/wap/goodsend.jsp?gdsid=<%= id %>" style="background:#f00; color:#fff; display:block; padding:3px; width:40px; text-align:center;">购买</a>
   ...
   <div style="background:#a52a2a; padding:2px; margin-top:2px;">
	   <form id="search_1" method="post" action="goodsdetail.jsp?act=search1">
...
<a href="/wap/flow.jsp">购物车</a>&nbsp;&nbsp;<a href="/wap/user/favorite.jsp">我的收藏</a><br/>
<a href="/mindex.jsp">首页</a>&nbsp;&nbsp;<a href="/wap/html/help.jsp">帮助</a>
<br/>
