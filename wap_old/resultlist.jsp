...
body{ background:#fff;color:#4b4b4b; padding-bottom:15px; line-height:18px; padding-left:4px; width:100%}
ul{list-style:none;}
...
<div class="all">
     <table>
		...
		if(count>0)
		{%><tr><td>
		  <a href="<%=request.getRequestURI() %>?productsort=<%=ps123%>&page=<%=pages-1 %>">上一页</a> 
		  <a href="<%=request.getRequestURI() %>?productsort=<%=ps123%>&page=<%=pages+1 %>">下一页</a>  
		  <form action="<%=request.getRequestURI() %>?productsort=<%=ps123%>&act=r" method="post">
		  <input type="text" id="pages" name="pages" style="width:50px;" value="<%= pages%>"/>/<%= pList1.size()%10>0?pList1.size()/10+1:pList1.size()/10 %>页<input type="submit" value="跳转"/>
		  </form>	
