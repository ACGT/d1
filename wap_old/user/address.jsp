...
<div style=" margin-bottom:15px;">
   <div style=" background:#FFDEAD; padding:3px; width:100%;">
	...
	if(list!=null&&list.size()>0){
		...
		for(UserAddress ua:list){
			...
			if(ua.getMbrcst_countryid().intValue()!=100 && Tools.isNull( ua.getMbrcst_memo())){
				i++;
				%>
				<input type="radio" name="address" value="<%= ua.getId()%>" <% if(i==1) out.print("checked");%> />
				<%=ua.getMbrcst_name() %>&nbsp;&nbsp; 
				<%=ua.getMbrcst_raddress() %>
				 &nbsp;&nbsp;  <a href="/wap/user/addressprovince.jsp?addressid=<%=ua.getId()%>&id=<%=ua.getMbrcst_provinceid()%>">修改</a>&nbsp;&nbsp;<a href="address.jsp?addressid=<%=ua.getId()%>&deladdress=1">删除</a>
				<br/>
    ...
    <div style=" background:#FFDEAD; padding:3px; width:100%; margin-top:7px;">
    <a href="addressprovince.jsp?op=add">新增</a>&nbsp;&nbsp;
    <br/>
    </div>
