...
<div style=" margin-bottom:15px;">
    ...
    <input id="hideProvId" name="hideProvId" type="hidden" value="<%=id %>" />
    <input id="hideCityId" name="hideCityId" type="hidden" value="<%=oldcityid %>" />
  <input id="hideAddressId" name="hideAddressId" type="hidden" value="<%=addressid %>" />
   ...
   <div id="second" style=" margin-top:2px; line-height:18px;">
	<span style="color:red;"><%=errormsg %></span> <br/>
	           第2步/共3步<br/>
		<span id="citytip" style="color:red;"></span>
		&nbsp;<font color='red'>*</font>&nbsp;县/市/区 <select id="ddlCity" name="ddlCity">
        	...
        	if(list != null && !list.isEmpty()){
        		...
        		for(City city : list){
        			%>
        			<option value="<%=city.getId()%>" <%if(id.equals(oldpid) && city.getId().equals(oldcityid)){ %>selected="selected"<%} %>><%=city.getCtymst_name()%></option>
        		 <%}
        	}
            %>
        </select><br/>
        <input type="submit" value="下一步" style="width:65px; padding:4px;"/>&nbsp;&nbsp;<a href="<%=backurl+"?"+url %>" style=" text-decoration:underline">返回上一步</a>
        </div>	
