...
<div style=" margin-bottom:15px;">
  ...
  <input id="hideOldProid" name="hideOldProid" type="hidden" value="<%=id %>" />
	  ...
	  <select id="ddlProvince" name="ddlProvince">
			...
			if(list != null && !list.isEmpty()){
				for(int i=0;i<list.size();i++){
					...
					%>
					<option value="<%=pro.getId()%>" <%if(id.equals(pro.getId())){ %>selected="selected"<%} %>><%=pro.getPrvmst_name()%></option>
				<%}
			}
			%>				
		</select>
		...
		<input type="submit" value="下一步"  style="width:65px; padding:4px;"/>&nbsp;&nbsp;<a href="<%=backurl %>"  style=" text-decoration:underline">返回上一步</a>
		</form>
		</div>				
