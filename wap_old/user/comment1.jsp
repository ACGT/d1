...
<%@ include file="../inc/head.jsp" %>
   ...
   <div style=" background:#FFDEAD; padding:3px; width:100%;">
    ...
    <div style="padding:5px;">
     <a href="comment1.jsp">待评价</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="comment1.jsp?type=1">已评价</a>
     <%
				 ...
				 if(!Tools.isNull(request.getParameter("pageno"))){
				  ...
				  PageBean pBean = new PageBean(totalcount,pagesize,currentPageIndex);
					 int end = pBean.getStart()+pagesize;
						...
						<tr>
						 <td class="d1" >订单号</td> <td class="d1" >操作</td>
						</tr>
					<%for(OrderBase base:nocommentlist2){
					%>
				 <tr><td  ><a href="/wap/user/orderdetail.jsp?orderid=<%=base.getId() %>" ><%=base.getId() %></a></td>
				   <td  ><a href="/wap/comment/addcomment1.jsp?orderid=<%=base.getId() %>" >确认收货并评价</a></td></tr>
...
<%} %>
 ...
 共 <b class="eng"><font color="#FF0000"><%=pagecount%></font></b> 页,当前第 <b class="eng"><font color="#FF0000"><%=currentPageIndex%></font></b> 页&nbsp;&nbsp;&nbsp;&nbsp;
              ...
              <a href="comment1.jsp?pageno=<%=currentPageIndex-1%>" ><font color="#666666">上一页</font></a> &nbsp;&nbsp;
              <%
		  }
				   ...
				   int i=0;
		 			  ...
		 			  String url="";
						 ...
						 int score=cache.getGdscom_level().intValue();
							...
							%>	
						 <tr ><td style=" color:#a25663;" >订单号</td>
						 <td ><a href="/wap/orderdetail.jsp?orderid=<%=cache.getGdscom_odrid() %>" ><%=cache.getGdscom_odrid() %></a></td>
						 </tr>	
						
		 			   <tr>
		 			    <td style=" color:#a25663;">商品信息</td>
			
 <td><span class="sptitle" style=" display:block; "><a href="<%=url %>" ><%=cache.getGdscom_gdsname() %></a></span></td>
 </tr>
 ...
 <div class="Pager">  
         共 <b class="eng"><font color="#FF0000"><%=pagecount%></font></b> 页,当前第 <b class="eng"><font color="#FF0000"><%=currentPageIndex%></font></b> 页&nbsp;&nbsp;&nbsp;&nbsp;
              <%if (currentPageIndex!=0) {%>
                  ...
                  <%}
              if (currentPageIndex>1 ){%>
              <a href="comment1.jsp?pageno2=<%=currentPageIndex-1%>&type=1" ><font color="#666666">上一页</font></a> &nbsp;&nbsp;
              <%
		  }
          	for(int j=pBean.getStartPage();j<=pBean.getEndPage()&&j<=pBean.getTotalPages();j++){
           		if(j==currentPageIndex){
